...
**`v0.4.3` 已于 2026-09-25 22:35 +0800 发布到预生产**（tag `v0.4.3` → `505feea`，镜像
`cps-novel:0.4.3-505feea`，`RELEASE=PASS`）。Owner 暂停正式领取批次后完成部署；admin2 已建号并追加到领取
UUID 白名单，独立 2FA 绑定与双账号认证核验待完成。正式领取批次由 Owner 自行恢复；真实领取提交验收另行授权。
领推广生命周期开关与按接口限速开关已在预生产开启，两个接口间隔仍各 1,500 ms；生产环境尚未上线。
