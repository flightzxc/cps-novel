...
### 3.4 Owner 授权例外：2026-09-06 Parity 收敛（Phase B–E）

🔴 **本节由 Owner 于 2026-09-06 明确授权，优先于 §3.1–§3.3 的按目录独占。**

背景：§3.1/§3.2 的 Claude / Codex 目录分权反映的是 2026-08 的协作模式。自 2026-09 起，实际分工已改为四层：Fable 编排与规划、Sonnet 施工、Opus 复核与打包、Codex 只读终审与发布。"短剧 → 海阅全链路 Parity 收敛"的各施工工单（Phase B 实体订正与运营表单 / Phase C 任务模型迁移与 ImportProgress / Phase D 安全与运行态收口 / Phase E 运营 E2E）按业务链路切分，必然横跨 §3.1 与 §3.2 两侧目录。

规则：
1. 在上述 Phase 的施工工单内，**工单点名的执行方即该工单所列路径的唯一写入者**，不论路径在 §3.1 还是 §3.2。"任何路径不得有两个写 Owner"仍然成立——唯一 Owner 由工单指定，而非由目录表指定。
2. 例外仅覆盖工单**明文列出**的文件与目录；工单未列出的路径仍按 §3.1–§3.3 执行。
3. 每个工单的交付必须经独立复核（Opus）方可合入；数据订正类操作由复核方执行，施工方只交付脚本与测试。
4. 本例外在 Phase E 收官（`NOVEL_OPERATIONAL_PARITY = READY`）后自动失效；届时 Owner 决定是把 §3.1–§3.3 重写为四层分工模型，还是恢复按目录独占。

已依此例外落地的工单：Phase B（提交 `03ae575`，2026-09-06）。
...
`v0.3.0` 于 2026-09-23、`v0.4.0` / `v0.4.1` 于 2026-09-24 发布到预生产；**`v0.4.2` 已于 2026-09-25 03:57 发布到预生产**
（tag `v0.4.2` → `33cd67b`，镜像 `cps-novel:0.4.2-33cd67b`，`RELEASE=PASS`），其中按接口限速开关默认关闭、开启须 Owner 另行批准。
领推广生命周期开关已于 2026-09-24 经 Owner 批准在预生产开启，正进行三级真实 UAT；生产环境尚未上线。范围、进度与后续以 `docs/governance/version-registry.md` 的「当前快照」为准，本节不复述。
