...
const actions = vi.hoisted(() => ({
  ...
  applyNovelMaterializeBatchAction: vi.fn(),
...
describe("BatchCreateContentDialog", () => {
  it("submits novel materialize without a template map", async () => {
    ...
    actions.applyNovelMaterializeBatchAction.mockResolvedValue({ ok: true, data: { taskId: "task-1", phase: "queued" } });
    ...
    await waitFor(() => expect(actions.applyNovelMaterializeBatchAction).toHaveBeenCalledWith(expect.objectContaining({ selection })));
    expect(actions.applyNovelMaterializeBatchAction.mock.calls[0][0].templateKeysByLocale).toBeUndefined();
  ...
  it("network rejection leaves the dialog retryable with the same request id", async () => {
    actions.readCatalogBatchContextAction.mockResolvedValue({ ok: true, data: context });
    actions.applyNovelMaterializeBatchAction.mockRejectedValueOnce(new Error("offline")).mockResolvedValueOnce({ ok: true, data: { taskId: "task-2", phase: "queued" } });
    ...
    fireEvent.click(screen.getByRole("button", { name: "纳入书目" }));
    await waitFor(() => expect(actions.applyNovelMaterializeBatchAction).toHaveBeenCalledTimes(2));
    expect(actions.applyNovelMaterializeBatchAction.mock.calls[0][0].requestId).toBe(actions.applyNovelMaterializeBatchAction.mock.calls[1][0].requestId);
  });

  it("keeps task link after summary failure", async () => {
    actions.readCatalogBatchContextAction.mockResolvedValue({ ok: true, data: context });
    actions.applyNovelMaterializeBatchAction.mockResolvedValue({ ok: true, data: { taskId: "task-3", phase: "queued" } });
  ...
  it("does not submit without a loaded context or publish permission", async () => {
    ...
    expect(actions.applyNovelMaterializeBatchAction).not.toHaveBeenCalled();
  });
});
