...
const NAV = [
  { label: "Home", href: "/dev-preview/home", current: true },
  { label: "Genres", href: "/dev-preview/collection" },
];
...
describe("页头", () => {
  ...
  it("不放 emoji 当 Logo", () => {
    const { container } = renderWithMessages(<SiteHeader navItems={NAV} />);
  ...
  it("活跃项用颜色 + 下划线双重编码，并标出 aria-current", () => {
    ...
    const active = screen.getAllByRole("link", { name: "Home" })[0];
...
describe("移动端导航", () => {
  it("默认收起，导航项不进入 Tab 序列", () => {
    ...
    expect(screen.getAllByRole("link", { name: "Genres" })).toHaveLength(1);
  });
  ...
  it("点击导航项后菜单收起", () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "Open menu" }));
    const mobileLink = screen.getAllByRole("link", { name: "Genres" })[1];
