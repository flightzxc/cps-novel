...
import { articleGenerateEligibleWhere } from "../../src/server/content-creation/eligibility";
import {
      ...
      protectedWrite: async (tx) => {
        ...
        if (!Number.isFinite(expiry) || Date.now() >= expiry) {
          await tx.genericTask.update({
            ...
            data: { result: { enumerationStatus: "expired", submittedCount: 0, childTaskCount: 0 } },
          });
        ...
        while (true) {
          ...
          submittedCount += novelIds.length;
          childTaskCount += 1;
          after = rows.at(-1)!.id;
        ...
        return {
          status: "success",
          result: { enumerationStatus: "completed", submittedCount, childTaskCount },
        };
      },
    };
  };
