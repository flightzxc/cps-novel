...
container_name="cps-novel-wo5-pg16-${run_id}"
database_name="cps_novel_wo5_${run_id//-/_}"
secret_dir="$(mktemp -d "${TMPDIR:-/tmp}/cps-novel-wo5-secrets.XXXXXX")"
cleanup_complete="no"
...
cleanup() {
  ...
  echo "WO5_DISPOSABLE_DATABASE_CLEANED=${cleanup_complete}"
}
...
docker run -d \
  ...
  --mount "type=bind,src=${secret_dir},dst=/run/wo5-secrets,readonly" \
  -e POSTGRES_USER=wo5_admin \
  -e POSTGRES_PASSWORD_FILE=/run/wo5-secrets/bootstrap-password \
  -e POSTGRES_DB=postgres \
  -p 127.0.0.1::5432 \
  postgres:16.14 >/dev/null

for _ in $(seq 1 60); do
  if docker exec "$container_name" pg_isready -U wo5_admin -d postgres >/dev/null 2>&1; then
    break
  fi
  sleep 1
done
docker exec "$container_name" pg_isready -U wo5_admin -d postgres >/dev/null
host_port="$(docker port "$container_name" 5432/tcp | tail -n 1 | sed 's/.*://')"

docker exec -i "$container_name" psql --no-psqlrc -U wo5_admin -d postgres \
  <"$project_root/infra/postgres/roles.sql" >/dev/null
docker exec -i "$container_name" psql --no-psqlrc -U wo5_admin -d postgres \
  <"$secret_dir/role-passwords.sql" >/dev/null
docker exec "$container_name" createdb -U wo5_admin -O migration_owner "$database_name"
...
WO5_DATABASE_TEST=1 \
WO5_OWNER_DATABASE_URL="$owner_url" \
WO5_WEB_DATABASE_URL="$web_url" \
WO5_WORKER_DATABASE_URL="$worker_url" \
WO5_SCHEDULER_DATABASE_URL="$scheduler_url" \
npm exec vitest run -- --project node tests/integration/tasks/worker-light-postgres.test.ts \
  --reporter=default --reporter=json --outputFile="$secret_dir/integration-result.json"

node - "$secret_dir/integration-result.json" "$project_root/tests/integration/tasks/worker-light-postgres.test.ts" <<'NODE'
...
if (files.length !== 1 || path.resolve(file?.name ?? "") !== expected
    || file.status !== "passed" || passed < 10 || skipped !== 0
    || report.numFailedTests !== 0 || report.numPassedTests !== passed) {
  console.error(`WO5_INTEGRATION=FAIL reason=not_executed passed=${passed} skipped=${skipped}`);
  process.exit(1);
}
console.log(`WO5_INTEGRATION=PASS passed=${passed} skipped=0`);
NODE

DATABASE_URL="$owner_url" node scripts/check-database-dictionary-drift.mjs

echo "WO5_DICTIONARY_DRIFT=0"
echo "WO5_POSTGRES_VERIFICATION=PASS"
