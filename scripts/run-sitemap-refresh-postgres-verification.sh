...
container_name="cps-novel-sitemap-refresh-pg16-${run_id}"
database_name="cps_novel_sitemap_refresh_${run_id//-/_}"
secret_dir="$(mktemp -d "${TMPDIR:-/tmp}/cps-novel-sitemap-refresh-secrets.XXXXXX")"
cleanup_complete="no"
...
cleanup() {
  ...
  echo "SITEMAP_REFRESH_DISPOSABLE_DATABASE_CLEANED=${cleanup_complete}"
}
...
docker run -d \
  ...
  --mount "type=bind,src=${secret_dir},dst=/run/sitemap-refresh-secrets,readonly" \
  -e POSTGRES_USER=sitemap_refresh_admin \
  -e POSTGRES_PASSWORD_FILE=/run/sitemap-refresh-secrets/bootstrap-password \
  -e POSTGRES_DB=postgres \
  -p 127.0.0.1::5432 \
  postgres:16.14 >/dev/null

for _ in $(seq 1 60); do
  if docker exec "$container_name" pg_isready -U sitemap_refresh_admin -d postgres >/dev/null 2>&1; then
    break
  fi
  sleep 1
done
docker exec "$container_name" pg_isready -U sitemap_refresh_admin -d postgres >/dev/null
host_port="$(docker port "$container_name" 5432/tcp | tail -n 1 | sed 's/.*://')"

docker exec -i "$container_name" psql --no-psqlrc -U sitemap_refresh_admin -d postgres \
  <"$project_root/infra/postgres/roles.sql" >/dev/null
docker exec -i "$container_name" psql --no-psqlrc -U sitemap_refresh_admin -d postgres \
  <"$secret_dir/role-passwords.sql" >/dev/null
docker exec "$container_name" createdb -U sitemap_refresh_admin -O migration_owner "$database_name"
...
echo "SITEMAP_REFRESH_DICTIONARY_DRIFT=0"
echo "SITEMAP_REFRESH_POSTGRES_VERIFICATION=PASS"
