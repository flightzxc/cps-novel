 ...
 * `"disabled"` for the exact value `disabled` (trimmed, case-insensitive).
 * Unset, `required`, a typo (`Disable`, `off`, `0`), or any other string all
 * resolve to `"required"` — the safe side a misconfigured environment falls
 * back to.
 */
...
/**
 * Only the exact (trimmed, case-insensitive) value `"disabled"` turns 2FA
 * off. Everything else — unset, `"required"`, a typo, an unrelated truthy
 * string like `"true"`/`"1"`/`"yes"`/`"off"` — resolves to `"required"`.
 * This is the inverse failure direction from most flags in
 * `src/lib/flags/feature-flags.ts`: those are *enable* flags that fail safe
 * by staying off on an unrecognized value; this is an *enforcement* flag
 * that must fail safe by staying ON (required) on an unrecognized value.
 */
