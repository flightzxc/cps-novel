...
export function CatalogSyncClient({ items, catalogGate, contentPublish, promoClaimGranted, promoClaimBlockedReason, templateOptions = [], filter = {}, total = 0 }: { items: readonly SourceItemRow[]; catalogGate: { readonly featureEnabled: boolean }; contentPublish: AdminCapabilityState; promoClaimGranted: boolean; promoClaimBlockedReason: string | null; templateOptions?: readonly { readonly id: string; readonly templateKey: string; readonly locale: string; readonly version: number }[]; filter?: CatalogFilterSnapshot; total?: number }) {
  ...
  return <div className="space-y-4">
    ...
    <div className="flex justify-between rounded-lg border border-gray-200 bg-gray-50 px-3 py-2"><p data-testid="promo-claim-toolbar-count">已勾选 <b>{selectedCount}</b> 条来源条目</p><div className="flex gap-2"><button disabled={!selectedCount} onClick={() => setCreateOpen(true)} className={buttonClassName("secondary")}>批量创建内容</button><button disabled={!selectedCount} onClick={() => setClaimOpen(true)} className={buttonClassName("primary")}>领取推广链接</button></div></div>
