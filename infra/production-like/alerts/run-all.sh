#!/usr/bin/env bash
# RC-7 minimal alerts (plus Gate 5-Dev's check-wal-archive.sh) — runs all four
# checks. Each check is independent: one failing (or alerting) never stops
# the others from running. Intended to be invoked from cron or the compose
# alerts wiring suggested in docs/operations/ALERTS_RUNBOOK_2026-09-03.md.
#
...
alert_log "run-all: check-wal-archive starting"
run_wal_archive_checks || failures=$(( failures + 1 ))
