...
执行期间源分支另行推进到 `ce3cada`（U6b 注释修订），上述 U6 backend 合并按固定输入未纳入它。
随后 Owner 追加授权，U6b 已通过 `72a991e` 合入本地 main；本页门禁计数仍对应原 U6 阶段，
追加合入及获批 Docker 缓存清理的独立实测见
[U6b 与 Docker 缓存清理回执](U6B_DOCKER_CACHE_2026-08-27.md)。
