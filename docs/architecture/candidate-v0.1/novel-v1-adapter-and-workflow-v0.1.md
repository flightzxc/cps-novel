# 海外阅读 v1 · Adapter 契约与工作流 v0.1（候选）
...
| **内容约束** | ① 单章长度上限（样本 3,997–11,858 字符），超限进审核队列不静默截断<br>② 单次返回条数上限（防上游异常返回巨量）<br>③ 正文不进日志、不进错误消息 |
...
| **状态** | 任务：`pending` → `processing` → `completed` / `completed_with_errors` / `failed`<br>item：`pending` → `processing` → `success` / `skipped` / `failed` |
| **幂等** | 同类型 active 任务**部分唯一索引**互斥；item 级 `(task_id, source_item_id)` 唯一 |
...
| 6 | dry-run 缺省 | `apply` 缺省 false。dry-run 走完全部只读判定并落判定结果 |

**分批策略（回答任务问题 9.1：9.5 万级目录怎么人工分批而不误触全量）**

这里有个必须正视的事实：`getlistpc` **没有已证的服务端筛选参数**（待决 D-2）。请求体只有 `name` / `orderType` / `pageIndex` / `pageSize` / `projectType`。这意味着"只同步分成≥50% 的书"**只能在拉回来之后过滤**。
...
**准入过滤（拉回后执行）**：
- `splitRatio >= 50` → 不满足则 item 落 `skipped`，记原因，**不建 SourceItem**
- `projectType == 1` → 交叉核验
- `agencyId` 在已登记 `ChannelApp` 中 → 否则 `skipped`，原因 `unregistered_agency`

**完整性核验**：抓取条数 vs `totalCount`。不足 → `partial_failed`。**不谎报 `completed`**。

| 项 | 内容 |
| --- | --- |
| **失败态** | item 级隔离，异常只毙该 item；attempt 在**领取时** +1；30 分钟未完成且 attempt<3 → 重置 `pending`，attempt≥3 → `failed(stale_processing)` |
| **人工介入** | `partial_failed` 可人工重试失败项；`skipped` 项可查原因 |
| **审计** | 批次级：参数、区间、抓取数、期望数、停止原因、耗时 |
| **验收** | ① dry-run 零写库；② 只开 Flag 不开 Allow Write → 明确失败且零写库；③ 命中安全上限 → `partial_failed`；④ 抓取数 < `totalCount` → `partial_failed`；⑤ 杀进程重启后未完成 item 恢复且 attempt 已 +1 |
...
| 项 | 内容 |
| --- | --- |
| **安全闸** | 单章长度上限；单次条数上限；正文不进日志/错误消息/审计 |
...
V1 **只存不用**。这是刻意的：Owner 已裁决 `taxonomy.rawSourceLabels = REQUIRED`、`phase2CanonicalMapping = PLANNED`。第一天不存，Phase 2 就永远缺历史数据。

### 3.5 内容审核

| 项 | 内容 |
| --- | --- |
| **触发** | 物化完成后自动进队列；或运营主动标记 |
| **状态** | `pending_review` → `approved` / `rejected` |
| **安全闸** | **未通过审核的内容不得发布**（发布门禁校验） |
| **人工介入** | 这条流程本身就是人工 |
| **审计** | 审核人、时间、结论、备注 |
| **验收** | 未审核内容无法通过发布入口 |
...
| 项 | 内容 |
| --- | --- |
| **安全闸** | Feature Flag；批量上限；未审核内容不可生成 |
| **失败态** | 单条失败不影响批次 |
| **人工介入** | 渲染失败队列 |
...
| 2 | 内容审核已通过 |
| 3 | `PromoLink` 存在且 `status=fetched`（**没有推广链接的页面不发布**——CPS 有明确先例：空 `promo_url` 的页面会产生 404 跳转） |
| 4 | slug 无冲突 |
| 5 | canonical URL 唯一 |
| 6 | hreflang 兄弟集合内所有目标均已发布或将同批发布 |
...
| **人工介入** | 门禁失败队列 |
| **审计** | 发布人、时间、门禁结果、URL |
| **验收** | ① 白名单外 locale 无法发布；② 空推广链接无法发布；③ 未审核内容无法发布 |
...
| **幂等** | `(url, revision)` 唯一 |
| **安全闸** | ① Feature Flag；② **页面类可索引判定**（见下）；③ **`NovelPreviewPolicy.index_authorized`，默认 false**；④ 投递失败有界重试 |

**页面类可索引判定（立项书 §3.4.4 已定，必须是代码级枚举 + 单一真源）**：
...
| 题材 / 榜单聚合页 | ✅ | 指向自身 |
| 试读**第 1 章** | ✅ | **指向书详情页** |
| 试读**第 2 章及以后** | ❌ noindex | **指向书详情页** |

章间用 `rel=prev/next`。**sitemap 不得包含作者/国家/完结类页面**（这类页面 V1 根本不存在）。

判定与授权是**与关系**：页面类允许索引 **且** 该书 `index_authorized=true`，才进 sitemap / IndexNow。
| **失败态** | 投递失败不影响发布，outbox 保留待重试 |
| **人工介入** | 后台可手动重推 |
| **审计** | 投递结果、HTTP 状态、耗时 |
| **验收** | ① 发布后 outbox 有行；② 搜索引擎接口不可用时发布仍成功；③ `index_authorized=false` 时试读页零推送；④ sitemap 静态产物只读服务，请求路径不触发生成 |
...
| **幂等** | `idempotency_key = (channel_app_id, external_book_id, source_language_code, channel_account_id)` upsert |
...
| 项 | 内容 |
| --- | --- |
| **安全闸** | canonical 投影需独立开关（**第三把钥匙**）；**canonical 只补空不覆盖** |
| **失败态** | `promo_not_generated` 不算失败 |
| **人工介入** | 无推广码的书进"待生成"清单，等 `claimPromo` 解冻 |
| **审计** | 脱敏：`[redacted_code:length=N]` + hostname |
| **验收** | ① 审计表中零真实推广码、零完整 URL；② 已有人工推广值不被覆盖；③ 幂等：重复运行不产生重复 PromoLink |
...
| --- | --- |
| **触发** | 读者点击 |
| **流程** | 按 `code` 查 `PromoLink` → 写埋点（异步/批量）→ 302 到 `web_url` |
| **幂等** | 读操作 |
| **安全闸** | ① `code` 查不到 → 稳定的 404 页，不泄露内部信息；② **目标 URL 必须过 host 白名单**（防开放重定向）；③ 埋点写入失败不影响跳转 |
| **失败态** | 埋点失败静默降级；跳转永不因埋点失败而失败 |
| **审计** | 埋点表本身即记录 |
| **验收** | ① 未知 code → 404；② 非白名单 host → 拒绝跳转；③ 埋点关停时跳转仍正常 |
...
| 场景 | 触发 | `Novel.status` | 页面响应 | 正文处置 | Sitemap/IndexNow |
| --- | --- | --- | --- | --- | --- |
| **下架** | 运营决定不再推广 | `unpublished` | 稳定的下架提示页 | 保留 | 从 sitemap 移除，推送 outbox |
| **版权撤回** | 上游/权利方要求 | `takedown` | 410 Gone | **删除 `NovelChapterContent` 行**，保留元数据 | 同上，且优先级最高 |
| **内容刷新** | 上游更新 / 定期 | 不变 | 不变 | 按 `content_hash` 判断是否重写 | 有实质变化才推送 |
| 🔴 **收费边界下调** | 上游 `payEpisFrom` 变小 | 不变 | 受影响章节页 **404** | **删除受影响章节的正文行**，章节置 `withdrawn` | 从 sitemap 移除受影响章节页 |

**收费边界下调是一条独立的、必须自动化的流程（立项书 §C.4 硬约束）**

> 若渠道上调 `payEpisFrom`（把原本免费的章节改成付费），**已展示章节必须自动下架，不得靠人工盯**。

```
每次目录同步 / 试读刷新时：
   新 payEpisFrom < NovelPreviewPolicy.last_paid_from_chapter ？
        ↓ 是
   受影响章节 = canonical_chapter_number >= 新 payEpisFrom
        ↓
   · NovelChapter.status → withdrawn
   · 删除对应 NovelChapterContent 行
   · 该章节页转 404
   · sitemap 移除 + IndexNow 推送
   · 写审计（不可逆操作，记录前后值与影响章号）
        ↓
   更新 last_paid_from_chapter
```

这是**合规刚性**，不是优化项——继续展示已转付费的正文是实质侵权。因此它**不受 Feature Flag 控制**：下架路径永远开着，只有"物化更多章节"的路径才受闸门控制。

验收（立项书 §14.2 P2-04）：模拟 `payEpisFrom` 从 7 改为 2，第 2–3 章页在下一个同步周期内返回 404。
...
**PG 形态**：部分唯一索引（`WHERE status IN ('pending','processing')`）。数据库约束是正确性边界，应用层检查只是快速失败路径。
...
任何实现方在交付渠道链路时，必须能演示以下 8 条：

1. 两个 worker 副本同时轮询，**同一任务只被执行一次**；
2. 两个并发入队请求，**只有一个建出 active task**，另一个明确失败；
3. 杀死 worker 后重启，未完成 item 在阈值后恢复为 `pending` 且 attempt 已 +1；
4. 只开 Feature Flag 不开 Allow Write，apply 模式明确失败且**零写库**；
5. dry-run 走完全部判定路径并落判定结果，**零上游副作用调用**；
6. 命中分页安全上限 → 任务 `partial_failed`，**不报 `completed`**；
7. 抓取数 < `totalCount` → `partial_failed`；
8. 审计表中**查不到任何真实推广码或完整链接**，只有 `[redacted_code:length=N]` 与 hostname。

第 4–8 条与 CPS 的既有验收口径一致。第 1–3 条是 PG 化新增，PG 原型已在沙箱证明可达成。
...
| 编号 | 未知项 | 阻塞什么 |
| --- | --- | --- |
| U-1 | `getlistpc` 是否支持服务端筛选 | 分批策略（待决 D-2） |
| U-2 | 一行 = 一本书还是一个语种版本 | Novel/SourceItem 基数（待决 D-1） |
| U-3 | 法语 `language` 数值枚举 | 法语同步不可执行 |
| U-4 | `chapterID` 稳定性 | 章节增量同步 |
| U-5 | 章节删除/卷层级/增量语义 | 内容刷新流程 |
| U-6 | `claimPromo` 全部合同 | 推广生成能力（待决，见开放决策 D-8） |
| U-7 | 收益书籍/推广码归因维度 | P4 解冻（待决 D-6） |
| U-8 | `GetReportMaxTime` 合同 | 收益日期边界（待决 D-5） |
| U-9 | 上游 429 / 限流阈值 | QPS 上限定值 |
| U-10 | `Areainterface` / `Browserlang` 请求头作用 | Adapter 请求头构造 |
| U-11 | `materialStatus` / `isRelease` / `localType` 枚举语义 | 无（V1 只存不用） |
| U-12 | 空页 / 最后一页返回形状 | 分页终止条件的健壮性 |
