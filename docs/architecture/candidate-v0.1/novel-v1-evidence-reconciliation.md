...
| 正文站内展示授权 | `preview.displayAuthorization = OWNER_CONFIRMED` | `P0_REVENUE_PROBE_AND_BACKFILL.md:180`（`OWNER_DECIDED`） | ✅ | `P0_SECOND_BROWSER_PROBE.md:169-171` 明确：Owner 确认 ≠ 渠道官方文档证据；**缓存、搜索引擎索引、完整/截断展示仍待书面确认** | **展示授权已开，索引/缓存/截断授权未开**。这三项进 Owner 待决（见 §6 D-3） |
...
| `splitRatio >= 50` 过滤 | Owner 已裁决 | `splitRatio` 是返回字段，未证明是请求筛选项 | 同上：过滤只能在**拉回来之后**做。这意味着"只同步分成≥50%的书"仍需遍历全目录。**这条直接决定分批策略**，见 `novel-v1-adapter-and-workflow-v0.1.md` §3.1 |
...
这不是折中，是 CPS 已经付过学费的形态：`v6.0.4` 的事故正是"只注册了前台 locale，漏了后台模板/文章创建枚举"（`docs/governance/version-registry.md`）。所以海外阅读第一天就要求：**一个语种要能公开，必须同时具备前台 messages、模板语种登记、SEO 元数据、sitemap 分片、hreflang 兄弟**——五项齐全才进白名单，缺一即不发布，且**默认不在白名单**。
...
| ID | 问题 | 为何现在必须问 | 状态 |
| --- | --- | --- | --- |
| **D-1** | `getlistpc` 的一行代表"一本书"还是"一本书的一个语种版本"？ | 决定 `Novel` ↔ `NovelSourceItem` 是 1:1 还是 1:N，进而决定同一本书多语种能否共享 canonical 身份 | OPEN |
...
| **D-3** | 试读正文的**缓存**、**搜索引擎索引**、**完整/截断展示**书面细则 | 立项书 §C.4 与 Gate C 尾巴。Owner 已确认可展示，这三项仍待补 | OPEN（Gate C 尾巴，不阻 P2） |
...
| 5 | 🔴 **`payEpisFrom` 上调时，已展示章节必须自动下架，不得靠人工盯** | §C.4 | **本方案原先漏了这条**。已补进内容刷新流程，见契约文档 §3.14 |
| 6 | 试读页 SEO 口径已定：**仅第 1 章可索引**，第 2 章起 noindex，全部 canonical 回详情页，章间 `rel=prev/next` | §3.4.4 | **本方案原先没写**。已补进架构文档 §6.1 与契约文档 §3.8 |
...
| W2 是否返回可展示正文 | "可以"（口头，`HYPOTHESIS`） | `P0_BROWSER_INTERFACE_PROBE.md:85` + Owner 确认 | 技术侧 `PRODUCTION_READ_PROVEN`；权利侧 `DOC_CONFIRMED`（Owner 声明，建议补登分销协议编号与有效期） |
