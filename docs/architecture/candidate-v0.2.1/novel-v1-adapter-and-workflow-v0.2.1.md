# 海外阅读 v1 · Adapter 契约与工作流 v0.2.1（候选）
...
> **v0.2.1 收口：2026-08-02。** 本文件变更点：**缺失章节处置冻结**（响应可信度前置判定 → 权威列表 → 旧章立即 stale / 重新返回自动恢复 / 正文不硬删；删除 K 次阈值与人工确认必选）；"完整试读目录"定义收窄；**`CatalogScanTask` 互斥改账户 × 应用 × projectType 单 active**；验收条目相应更新。逐项见 `CHANGELOG-v0.2-to-v0.2.1.md`。
...
| **幂等** | 🔴 **v0.2.1 目录扫描互斥**：`UNIQUE(channel_account_id, channel_app_id, project_type) WHERE status IN ('pending','processing')`——**同一账户 × 应用 × 品类同时只允许一个 active catalog scan**，不用 page-range hash（分页区间不是独立作用域，重叠并发会争配额、互相覆盖 upsert、污染水位统计）。要扫更多页就排队跑下一批，不并发。item 级 `(task_id, page_index)` 唯一 |
...
系统拒绝的输入：
  ✗ 不填区间（"同步全部"）
  ✗ 区间跨度超过单任务安全上限
  ✗ 同一 channel_account + channel_app + projectType 已有 active catalog scan
     （v0.2.1：目录扫描单 active，不按页区间放行并发）
```
...
调 fetchPreviewChapters
   ↓
返回校验：HTTP 200 且结构完整 且 chapterList 非空
   ↓  否 → 记 errorKind + 审计；**不改变任何既有章节的可见状态**（v0.2.1）
逐条处理（i 升序）
   ↓
   ...
   ↓
   ↓
差异处置：权威列表中缺席的旧章 → 立即 stale；已 stale 且重新出现 → 自动恢复
   ↓
更新 NovelPreviewPolicy.materialized_chapter_count（结果记录）
```

**物化产物同时构成「完整试读目录」的数据源（v0.2.1 定义）**：

```text
完整试读目录 = 展示 chapterList[] 当前实际返回的全部章节
```

不宣称拥有全书目录；**不得根据 `allEpis` 生成无标题章节占位**；目录完整性只与当前权威列表一致，不与 `allEpis` 比对。目录的承载形式（独立路由 vs 嵌入详情页）是待决 **D-12**。
...
**刷新与缺失处置（v0.2.1 · `OWNER_DECIDED`，D-10 已关闭）**

规则的关键是**顺序**：先判响应是否可信，再谈集合差异。

```
第一步 · 响应可信度判定
  请求失败 / 响应结构异常 / 异常空列表
      → 不改变现有可见状态
        （不 stale、不下架、不删除、不更新正文；仅记一次失败观测与审计）
      → 本轮结束

  请求成功 且 结构完整 且 chapterList 非空
      → 该列表即「当前权威试读集合」，进入第二步
...
第二步 · 与权威集合比对
  ...
  新章出现        → 新增物化 + 新增可索引章节页
  旧章不在列表中  → 立即 status=stale：停止展示、退出 sitemap、推送移除
  已 stale 的章重新出现 → 自动恢复 status=preview：恢复展示与收录（写审计）

任何情况下：
  正文保留，不自动硬删（硬删仅属人工发起的版权撤回）
```

**v0.2.1 相对 v0.2 删除的两条**：
- ⛔ **连续 K 次缺失阈值**（`consecutive_miss_count` 字段一并删除）；
- ⛔ **人工确认作为下架或恢复的必选环节**（人工队列降为可选运维视图，不再阻断状态流转）。

**为什么可以去掉 K 次缓冲**：缓冲原本用于吸收上游抖动，而抖动的表现恰恰是请求失败、结构异常、异常空列表——这些情形在第一步就被拦下且不改状态。能进入第二步的必然是一个结构完整且非空的成功响应，它就是当前权威，没有再等 K 次的理由。**稳健性由"响应可信度前置判定"提供，不由计数器提供。**

这仍与 CPS 同构：纯 upsert 状态标记、全文件无删除路径（`preview-sync.ts:379-409`），`stale` 在其状态枚举中已预留（`:30`）。策略"等上游明确下架状态"依然不可行——`getchapterinfo` 无删除/下架字段（探测已证）。

⛔ **v0.2 删除并在 v0.2.1 保持删除**：v0.1 的"`payEpisFrom` 下调 → 自动撤回章节"路径。`payEpisFrom` 只是收费边界元数据；上游若真收回试读资格，表现必然是该章从 `chapterList[]` 消失，自然落入上面的缺失处置流程。
...
| **章节脱离权威列表**（v0.2.1 已冻结） | 一次**可信**（成功+结构完整+非空）响应中该章未出现 | 章节 → `stale`，**立即** | 该章节页停止展示 | **保留正文行**（不自动删） | 从 sitemap 移除 + 推送移除 |
| **章节重新返回**（v0.2.1 新增） | 该章重新出现在权威列表中 | `stale` → `preview`，**自动** | 恢复展示 | 正文原样复用 | 重新进 sitemap + 推送 |
| **响应不可信** | 请求失败 / 结构异常 / 异常空列表 | **不变** | **不变** | **不变** | **不变** |

⛔ **v0.2 删除：v0.1 的"收费边界下调 → 自动删除正文"整条流程。**

Owner 裁决（证据文档 §8.4）：`payEpisFrom` 只是渠道收费边界元数据，其数值变化**不得触发**自动删除；试读集合的唯一真源是 `chapterList[]` 实际返回。若上游真的收回某章试读资格，该章会从 `chapterList[]` 消失，正确地落入上面"章节脱离权威列表"的处置（立即 stale 停展、正文保留、重新返回自动恢复）。v0.1 依据的立项书 §C.4 条目应按被替代机制标记，**除非后续获得渠道正式合同（`DOC_CONFIRMED`）证明 `payEpisFrom` 与试读资格回收的因果语义，否则不得恢复**。
...
⛔ v0.1 的"全局同类型只许一个 active"过宽——会让账户 A 的作业挡住账户 B 的无关作业。`operation_scope_hash` 是操作范围（目标集合 / projectType 等）的规范化指纹；重叠但 hash 不同的情况由入队工厂做应用层预检，数据库约束兜住完全相同的重复提交。数据库约束仍是正确性边界，应用层检查只是快速失败路径。

🔴 **v0.2.1 例外：`CatalogScanTask` 不适用本规则。** 目录扫描改为

```text
UNIQUE(channel_account_id, channel_app_id, project_type) WHERE status IN ('pending','processing')
```

即**同一账户 × 应用 × 品类同时只允许一个 active catalog scan**，不按 page-range hash 放行并发。分页区间不是独立作用域——同一目录的并发扫描会争上游配额、互相覆盖 `NovelSourceItem` upsert、污染 `catalog_observed_total` 水位与批次统计。要扫更多页就排队跑下一批。
...
## 6. 验收口径（工程原型级，v0.2.1 修订）

任何实现方在交付渠道链路时，必须能演示以下 15 条：

1. 两个 worker 副本同时轮询，**同一任务只被执行一次**；
2. 两个并发入队请求（同账户同应用同范围），**只有一个建出 active task**，另一个明确失败；**不同账户或不重叠范围的两个任务可同时 active**（v0.2 作用域化，目录扫描除外见 #13）；
...
11. **响应不可信时状态不变**（v0.2.1）：构造请求失败 / 结构异常 / 异常空列表三种情形，已展示章节**保持展示**，无 stale、无下架、无删除；
12. **旧章脱离权威列表立即 stale 且正文仍在**（v0.2.1）：一次成功且非空的响应中缺某章 → 该章立即停展、退出 sitemap，`NovelChapterContent` 行仍存在；
13. **stale 章重新返回自动恢复**（v0.2.1）：该章重新出现在权威列表 → 自动回 `preview`、恢复展示与收录，**无需任何人工确认**；
14. **目录扫描单 active**（v0.2.1）：同一 `channel_account + channel_app + projectType` 已有 active catalog scan 时，另一个不同页区间的扫描入队**被拒绝**；不同账户或不同 projectType 可并行；
15. **无异常批次零人工触点**：从同步到发布全程不需要任何"逐条审核通过"操作（v0.2）。

第 4–8 条与 CPS 的既有验收口径一致（口径已按 v0.2 修正）。第 1–3 条是 PG 化新增，PG 原型已在沙箱证明可达成。第 9–10、15 条是 v0.2 Owner 裁决的验收化；**第 11–14 条是 v0.2.1 收口的验收化**（D-10 冻结语义 + 目录扫描互斥）。

⛔ **已删除的 v0.2 验收项**：原第 11 条"模拟某章连续缺失，第 K 次后转 stale"——K 次阈值已随 D-10 关闭而删除，由上面第 11–13 条取代。
...
| U-5 | 章节删除/卷层级/增量语义 | ~~缺失处置策略~~ | **v0.2.1 不再阻塞**：处置规则已冻结且**不依赖**上游给出删除语义——判据是"该章是否在最近一次可信响应的列表中"。上游语义本身仍未知，但已无待决事项挂在它上面 |
