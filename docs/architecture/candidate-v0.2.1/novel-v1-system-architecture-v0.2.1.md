# 海外阅读 v1 · 总体技术架构 v0.2.1（候选）

> 文档性质：**架构候选**，待 Owner 审核。非 Owner 已确认。
> v0.2 修订：2026-08-02（Owner 裁决回灌 + CPS 同构性复核）；**v0.2.1 收口：2026-08-02**（目录定义收窄 · D-10/D-11 关闭 · 旧口径清理 · 扫描互斥修正）。逐项变更见 `CHANGELOG-v0.1-to-v0.2.md` 与 `CHANGELOG-v0.2-to-v0.2.1.md`。
> CPS 代码基线：`d77c3b968285698529cf97c7f0f97b286d7a2a9c`（v8.1.1，树干净）
> 配套文档：`novel-v1-evidence-reconciliation.md`（证据，含 §8 v0.2 裁决回灌）、`novel-v1-logical-data-model-v0.2.1.md`（数据模型）、`novel-v1-adapter-and-workflow-v0.2.1.md`（契约与流程）、`novel-v1-implementation-plan-v0.2.1.md`（阶段计划）、`novel-v1-open-decisions.md`（待决）、`novel-v1-cps-parity-review.md`（CPS 五项同构性复核）
...
1. **进货**——运营发起一个批次，系统从上游拉一段目录回来，存成"来源条目"。**所有正常返回的内容都收**（`ACCEPT_CHANNEL_CONTENT`，Owner v0.2 裁决）：分成比例只是渠道业务属性，不是准入门槛。
2. **上架**——把来源条目转成站内的权威内容实体，生成落地页（每个来源语种版本一个独立 Novel），推给搜索引擎。
3. **挂链**——把上游**已有**的推广资源绑到内容上，并为每条资产分配我方公开短码；前台跳转只暴露公开短码，绝不暴露渠道真实码。**这一步在 v0.2 已并入内容上线主链**（已有资源在目录返回里就有，零额外上游请求）。
4. **对账**——回拉收益，归因到书和推广码。

V1 交付前三步。**第四步只留占位，不建表**——因为收益的归因维度还没探明（详见证据文档 §3.4）；但多账户模型（`ChannelAccount 1..N` + `RevenueSyncScope`）从 Day 0 就是 Schema 形状，见数据模型 §2.16。
...
| **tracking / `/go/:code`** | `cps-tracking.ts` `tracking.ts` `src/app/go/[code]/route.ts` | ✅ 事件表结构（6 个索引设计良好）；`hashSensitive` 盐哈希（`:284`，IP/UA 不存原值） | 汇总表去 `beidou_*` 列；**写入策略重设计**（当前每事件一次 `create`，PG 下应批量写 + 独立 flush） | — | ⚠️ **`/go` 的码语义不搬**：CPS 直接把渠道真实推广码放公开 URL（`route.ts:94-98`）且无唯一约束（`schema.prisma:32`）；`settlementOrder` 回填分支 | **`public_redirect_code`**：我方公开短码，唯一生成入口（改造 `article-public-page-id.ts:32-44` 模式），DB 唯一 + 不可变 | Owner v0.2 裁决拆分两码；详见 parity 报告 §3 |
...
> v0.2 变更：原第 5 项含"hreflang 兄弟链路已验证"，随 Owner 裁决（V1 不生成跨 Novel hreflang，见证据文档 §8.2）移除。每个语种版本是独立 Novel、独立页面体系，没有翻译兄弟集合，hreflang 无对象。CPS 复核确认这与其现状同构（parity 报告 §1）。
> **v0.2.1 收口**：发布白名单的验收清单中**不含任何 hreflang 项**，全文档不再出现"跨语种关系用 hreflang 表达"的表述。
...
| **hreflang** | **V1 两个方案下都不生成跨 Novel hreflang**（Owner 裁决：语种版本互为独立作品页，无翻译兄弟）。此行仅保留给 Post-V1 `NovelWork` 引入后重估 | 同左 |
...
**硬约束（不可协商）**：无论选哪个方案，**不同语言绝不共用同一个 canonical URL**。每个语种版本（独立 Novel）有且只有一套自己的 canonical URL，指向自身。v0.2 起：**V1 不存在需要表达的跨语种关系**——不合并、不互指、不推断；`NovelWork / TranslationGroup` 留给 Post-V1。
...
| **试读目录** | 展示 `chapterList[]` **当前实际返回的全部章节**（见下方定义）。承载形式二选一由 Owner 定（待决 **D-12**）：独立目录路由，或嵌入详情页 |
...
| SEO metadata | title / description / canonical / OG / JSON-LD（V1 无跨语种 hreflang） |

#### 「完整试读目录」的定义（v0.2.1 收口）
...
1. **不宣称拥有全书完整章节目录。** 页面文案、结构化数据、SEO title/description 均不得表述或暗示"全书目录"；对外语义是"可试读章节"。
2. **不得根据 `allEpis` 生成无标题章节占位。** `allEpis` 只是元数据标量（详情页可展示"共 N 章"这一事实），但不得据此产出任何章节行、列表项或页面骨架——目录里的每一条都必须有上游实际返回的章节数据支撑。
3. **目录完整性口径 = 与当前 `chapterList[]` 一致**，不与 `allEpis` 比对，也不因两者不等而报缺失或告警。

**承载形式待 Owner 选择（待决 D-12）**：① 独立目录路由（如 `…/novel/{id}/chapters`，自身可索引、可进 sitemap）；② 目录嵌入详情页（不新增路由，详情页内一段列表）。两种形态下上述三条约束都成立；本轮不替 Owner 选。

**试读页的 SEO 口径（v0.2 · Owner 已裁决 `OWNER_DECIDED`，取代立项书 §3.4.4 的"仅第 1 章可索引"）**：

| 页面类 | 口径 |
| --- | --- |
| 书详情页、题材/榜单聚合页 | **可索引**，self-canonical |
| **全部实际物化的试读章节页** | **可索引**，**self-canonical**，可进 sitemap 与 IndexNow |
| 试读目录（若选独立路由） | **可索引**，self-canonical；若选嵌入详情页则随详情页 |
| 章节页的结构要求 | 链回详情页；上一章/下一章导航；完整 CTA；标记所属书籍（结构化数据 `isPartOf`）；title/description 按"书名 + 章名/章号"模式生成，避免重复 |
| 页面关系 | `Novel detail ├─（目录）├─ Chapter 1 ├─ Chapter 2 └─ Chapter N`——每页独立 SEO 页面，**不把章节页 canonical 指向详情页** |

"前三章"只是当前三本、两语种生产样本的观察值。**上游实际返回 N 章 → 生成 N 个可索引章节页**；另设异常安全上限（单本物化章节页数上限，可配置），防止上游异常返回大量正文时无限生成页面。

🔴 **可索引判定必须是代码级枚举 + 单一真源，不能是模板里的 `if`。** 立项书 §3.4.4 已指出：CPS 仓库里并不存在所谓"A/B/C 页面分类"，实际只有四条互相独立的机制（`Article.seoVisibility` 取 `public`/`seo_only`——后者是 index,follow 但不进列表页，**不是** noindex；分页 ≥2 的 noindex；`unavailable-pages.ts` 下架清单；`Drama.rightsStatus` 的 410/404 门禁）。海外阅读不能重复这种"口头约定分散实现"的形态——即使 v0.2 的口径是"全部可索引"，判定也要走同一个枚举真源，因为 `stale`/下架/撤回状态仍会把单页翻转为不可索引。

**`NovelPreviewPolicy` 的角色变化（v0.2）**：`display_authorized` / `index_authorized` 不再是"默认 false 等授权"的闸——Owner 已裁决试读整体进入 SEO，两者默认放行；字段保留为**单本覆盖开关**（下架、撤回、争议处置时可单本翻转），fail-open 变为默认、fail-closed 变为例外处置。
...
| 书目列表 | 来源条目列表，筛选、状态、同步时间、`splitRatio`（**运营筛选维度，非准入门槛**） |
| 小说详情（后台） | canonical 实体编辑、来源条目关联、标签、发布状态 |
| 试读管理 | 已物化章节查看、重新拉取、`stale`/缺失状态处置（**v0.2：无逐条人工审核环节**，只有异常处理） |
| 已有推广资源 | 读取 + 展示 + 绑定到 PromoLink + **公开短码分配**（**已证能力，v0.2 移入内容主链**） |
| 推广生成 | **Owner Gate 形态**：按钮存在但由能力注册表控制为 `disabled`，展示原因 `capability_contract_unproven`（**未证能力**） |
| Article 模板 | 模板 CRUD、语种、变量白名单、渲染预览 |
| 发布 | 单条 / 批量发布，**机器门禁校验（无逐条人工审核）**；异常条目进人工处理队列 |
| 任务中心 | 任务与 item 两级列表、状态、attempt、失败原因、人工重试、异常处理队列（含 `claim_retry_blocked` 的 manual review） |
...
作者页、国家页、完结/连载榜、C 端登录、完整阅读器（付费章节）、收益正式页面（等待归因维度确认）、站内搜索（Post-V1 候选）、评论/评分、用户收藏、跨语种作品关联与 hreflang（Post-V1 `NovelWork`）、逐条人工内容审核。
