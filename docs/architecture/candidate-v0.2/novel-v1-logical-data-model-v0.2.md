...
| **外键** | `novel_id → Novel` |
| **状态** | `preview`（试读）/ `locked`（付费，V1 不物化）/ `stale`（v0.2 新增：连续缺失停展待人工，见 parity 报告 §5 推荐，`CANDIDATE` 待冻结）/ `withdrawn`（人工撤回） |
...
| **v0.2 候选字段** | `last_seen_at`（最近一次在上游返回中出现）+ `consecutive_miss_count`（连续缺失计数）——服务"连续 K 次缺失 → `stale`"的推荐策略（`CANDIDATE`，待 Owner 冻结缺失处置方案后定稿） |
...
- 若上游真的收回某章的试读资格，表现必然是**该章不再出现在 `chapterList[]` 里**——这正确地落入"缺失章节处置"流程（连续缺失 → `stale` 停展 + 人工队列，见 `novel-v1-cps-parity-review.md` §5 的推荐），而不是靠一个未经渠道合同证实的字段语义推断。
...
| **唯一键** | active 互斥见下方 v0.2 作用域规则 | `(task_id, page_index)`（或 page-range 起点） |
| **外键** | `channel_app_id`、`channel_account_id` | `task_id` |
| **字段** | 页区间起止、`catalog_observed_total`（本次观察到的 totalCount 水位）、`batch_expected_count` / `batch_actual_count` | `page_index` / `page_range`、`request_fingerprint`、返回行数、错误 |
...
**Active Task 互斥作用域（v0.2 修正）**：v0.1 写的"同类型 active 任务部分唯一"过宽——它会让"账户 A 扫 1–50 页"挡住"账户 B 扫 200–250 页"这类完全无关的并行。修正为：
...
`operation_scope_hash` = 操作范围指纹（页码区间 / projectType / 目标集合的规范化哈希）。同一账户同一应用同一范围只许一个 active；不同账户、不同应用、不重叠范围可并行。范围重叠检测（区间相交但 hash 不同）由入队工厂做应用层预检，数据库约束兜住完全相同的重复提交。
