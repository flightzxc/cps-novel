# 海外阅读 v0.2 · CPS 短剧同构性只读复核

> 文档性质：**只读代码核实报告**，为 candidate-v0.2 的修订提供 CPS 现状证据。
...
| 4 | 推广值存储分布 | 真实值受控存在于 4 处；后台明文可见；写路径锁定；审计脱敏 | `CPS_CURRENT_STORAGE_BEHAVIOR` 见 §4，推荐方案 A |
| 5 | 缺失内容下架 | **纯 upsert 状态标记，无删除路径**；`stale` 枚举保留但从不写入 | 支持"标记不删"方向，推荐见 §5 |
...
**v0.2 采用方案 A 作为候选推荐（`CANDIDATE`），等待 Owner 确认。** candidate-v0.1 中"普通角色只见掩码"的表述**撤回**（超出 CPS 实践且无事故依据）。
...
| 策略 | 评估 |
| --- | --- |
| ① 一次缺失立即下架 | ❌ 不推荐。上游一次抖动（超时、部分返回）就撤掉在售页面；CPS 也没这么做 |
| ② 连续多次缺失后下架 | ✅ **推荐主体**。连续 K 次（建议 K=3，可配）刷新未返回 → 章节置 `stale` 并停止前台展示 |
| ③ 标记 stale，人工处理 | ✅ **推荐并入**：置 `stale` 的同时进人工队列；`stale` 是"停止展示 + 待人工"，不是删除 |
| ④ 明确上游状态出现后才下架 | 理想但**不可行**：上游 `getchapterinfo` 无删除/下架字段（探测已证：删除态、卷层级、增量语义均未观察到），无状态可等 |

**推荐（`CANDIDATE`，本轮不冻结）**：②+③ 组合——

```
刷新时该章仍返回：
  hash 未变 → 不重写
  hash 变化 → 更新正文，consecutive_miss_count 清零
刷新时该章未返回：
  consecutive_miss_count += 1，记 last_seen_at
  count < K → 不动，继续展示
  count ≥ K → NovelChapter.status = stale：停止前台展示、退出 sitemap、进人工队列
任何情况下：
  不自动硬删除 NovelChapterContent 行（硬删只发生在版权撤回流程，人工发起）
```

与 CPS 的"状态标记 + 展示过滤 + 永不因单次缺失删数据"完全同构。数据模型侧需在 `NovelChapterSourceItem` 增加 `last_seen_at` 与 `consecutive_miss_count` 两个候选字段。
