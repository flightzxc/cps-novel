# 海外阅读 v1 · 证据收敛表（candidate-v0.2）

> 文档性质：**架构候选（candidate）**，非 Owner 已确认。
> v0.1 生成：2026-08-02；**v0.2 修订：2026-08-02（Owner 裁决回灌 + CPS 同构性复核）**
...
刷新语义：同章仍返回且 hash 未变 → 不重写；hash 变化 → 更新；新章 → 新增；**某章一次未返回不得立即硬删**。缺失章节处置的四案比较与推荐（连续 K 次缺失 → `stale` 停展 + 人工队列，绝不自动硬删正文）见 parity 报告 §5——CPS 预览刷新正是"纯 upsert 状态标记、无删除路径"（`preview-sync.ts:379-409`），推荐与其同构。**本轮只推荐，不冻结。**
...
### 8.7 推广资产与 raw payload 存储遵循 CPS
...
**推荐方案 A（严格复用 CPS，`CANDIDATE` 待 Owner 确认）**；v0.1 的"普通角色只见掩码"表述**撤回**（超出 CPS 实践且无事故依据）。方案 B（收紧读权限）仅当未来出现读侧泄露事实时再议。
...
| 5 | Active Task 互斥 | 作用域从"全局同类型"改为 `(task_type, channel_account_id, channel_app_id, operation_scope_hash)` |
