...
- Carousel serving source（`home_carousel_serving.source`，机器真源 `CAROUSEL_SOURCES`）：
  `manual | new_novel | recency`（`20260912100000_carousel_serving_source_check_fix` 修正——原
  CHECK 是 `manual | automatic` 两桶集合，`computeHomeCarouselInTx` 从落地起就只写细分值
  `manual`/`new_novel`/`recency`，从未写过 `automatic`，导致每次自动位计算的
  `homeCarouselServing.createMany()` 都以 PostgreSQL 23514 失败；CPS 对应列
  `home_carousel_serving.source`（`3a76877:prisma/schema.prisma:825`）本身没有 CHECK 约束，
  `mergeCarouselServingInTx` 直接写 `manual`/`candidate.source`（`new_drama|recency|revenue`）
  ——细节见 §12 本行 changelog 与迁移文件自身注释。`home_carousel_auto_candidate.source` 同样
  只取 `new_novel | recency` 两值，无 CHECK，未受此次改动影响）
...
| 2026-09-10 | L10N P3 模板 locale 非空化 + 15 语默认模板资产（`施工提示词_Sonnet_L10N_P3_模板locale非空化与15语模板资产_2026-09-10.md`，矩阵 #5，`20260912090000_l10n_article_template_locale_not_null`） | 一条迁移 + 服务层收口 + 15 份新增资产文件，四件事：(1) 迁移先 `UPDATE "article_template" SET "locale" = 'en' WHERE "locale" IS NULL`（X8 `cps-novel-x8-local-postgres-1` 施工首步实测 `SELECT count(*) FROM article_template WHERE locale IS NULL` = 0，该表总行数本身也是 0，回填是防御性安全网非本次生效路径），再 `ALTER COLUMN "locale" SET DEFAULT 'en', SET NOT NULL`；`prisma/schema.prisma` 同步改 `locale String @default("en") @db.VarChar(16)`（CPS 同型 `3a76877:prisma/schema.prisma:518`）。(2) `src/server/article-templates/service.ts`：`requireLocale` 删除 null/空白="全部语种"语义，只接受 `SITE_LOCALES` 15 项成员；非成员（含 `it`/`tr` 这类真实但非站点语种的 BCP-47 码）与格式错误串一样拒绝；`selectActiveArticleTemplate`/`listActiveArticleTemplateOptions` 各自删掉一处 `OR: [{locale:X},{locale:null}]` 通配，改精确 `{locale:X}`；`ensureDefaultArticleTemplate` 逐字未改（仍只 bootstrap en 种子行）。(3) `src/app/(admin)/templates/_components/template-manager.tsx`：模板语种下拉收口为 `SITE_LOCALES` 15 项（不抄 CPS `TEMPLATE_LOCALE_OPTIONS` 17 项，那 17 项相对本仓 `SITE_LOCALES` 多出 `it`/`tr` 两个真实但未登记为站点语种的 BCP-47 码——不是 `pt`/`zh-TW` 别名折叠，本仓不登记为独立 locale）、删「全部语种」选项、加 `required`；`TemplateRow.locale` 类型随 Prisma 生成类型收窄为 `string`（不再 `string \| null`）。(4) 新增 `assets/article-templates/` 15 份默认模板资产（`en.json` 逐字导出现行 `system-default-v1` 种子行——`DEFAULT_ARTICLE_TEMPLATE`/`SYSTEM_DEFAULT_CONTENT_BLOCKS` 常量，其余 14 语种翻译 4 处自然语言短语——封面 alt/总章节数前缀/免费试读前缀/CTA 文案，变量占位符与 `{if}/{endif}` 控制标记逐字不变）+ `manifest.json`（15 条 SHA-256）+ `scripts/l10n/article-template-bootstrap.ts`（dry-run 默认、SHA 钉死、结构不变量交叉校验、`--apply --approver` 落库、按 `(templateKey, version)` upsert 幂等、自建 `OperationAudit` 记 manifest SHA provenance，形状 `PATTERN_ONLY` 复用 `tagging-bootstrap.ts`，登记见 `port-registry.md` L10N P3 节）。`database-schema-dictionary.jsonl`：`db:public:article_template:locale` 记录原地更新 `nullable: true→false`、`default: null→"en"`，`evidence` 追加本迁移文件引用，`introduced_in_migration` 保持原值不变（同 P2-02B 对 `article_template_status_check`、C-27 对 `article.novel_id` 的先例，未 supersede）；§3.3 `article_template` 表行描述同步补一句。范围外未动：`PUBLISHABLE_LOCALES`/`ARTICLE_TEMPLATE_CRUD_LANDED`/`proxy.ts`（P4 范畴）、`content-creation`/`catalog-sync`/`publish-gate`（P2 并行占用）、`articles/page.tsx:149`（P5 范畴，仍硬编码 `"en"` 调 `listActiveArticleTemplateOptions`，该调用点传字面量字符串不受 Prisma 类型收窄影响，故未改也未破）。 | Claude Fable 5.1（编排）/Sonnet 5（施工） | 独立 worktree `l10n-p3-templates`（`feature/l10n-p3-template-locale`，基线 `feature/l10n-full-cps-parity@34390f8`）：`DATABASE_URL=postgresql://x:y@localhost:5432/z npx prisma validate` PASS；`npm run typecheck`（0 error）；`npm run lint`（0 error，4 条既有警告，与改动前基线数量逐字相同）；`npm run test:ui`（139 files / 2352 passed）；`npm run test:backend`（215 files，214 passed / 1 failed——唯一失败 `tests/backend/publish-gate/no-bypass.test.ts`，已用 A/B 复核：`git stash` 回到本轮改动前跑同一条命令命中同一签名 `scripts/s1-exact-target-structural-smoke.ts` 的 raw SQL 探测，逐字相同，与本轮改动无关）；`npm run test:integration`（21 files，2 可跑文件全过、19 个需要真实 PostgreSQL 连接的文件按既有 `C30_DATABASE_TEST` 同款约定跳过）；`node scripts/check-database-dictionary-drift.mjs --static`（`{"status":"ok","models":52,"recordCount":1208,"activeCount":1138}`）。四条变异逐条做过、贴红、`git checkout -- .` 回滚并核对 `git status` 与回滚前 staged 状态逐字一致：①服务层加回 `OR {locale:null}` 通配 → `tests/backend/article-templates/service.test.ts` 新增的孤儿行判死测试红；②`article-template-bootstrap.ts` 去掉 SHA 校验 → `tests/backend/locale/article-template-bootstrap.test.ts` 的 SHA 不符测试红（"expected function to throw an error, but it didn't"）；③真实 `ru.json` 删一个 `{total_chapter_count}` 占位符（manifest SHA 同步更新到位，排除单纯 SHA 不符干扰）→ 真读资产的 7 个测试红 + 实跑 `npx tsx scripts/l10n/article-template-bootstrap.ts`（无需数据库连接即在加载阶段报错）红；④`requireLocale` 放行空字符串 → 3 个测试红（空串/null/undefined 均从拒绝变成误放行）。**一次性 PostgreSQL 16 容器 `migrate deploy` 幂等重放 + 双向 `migrate diff --exit-code` 零差异 + `check-database-dictionary-drift.mjs`（非 `--static`）+ 真实 bootstrap CLI 对真库的 dry-run/apply 演练均待后续执行方补跑**（本轮按 X8 只 SELECT 的纪律未对任何数据库执行写操作，X8 现存 `article_template` 表结构仍是迁移前的可空版本），本行状态到那之前不得视为"已验证"，与 C-24/C-27/C-30A 三行同一措辞、同一约束。 |
| 2026-09-11 | L10N P5.2 scheduler 缺口收口（矩阵 #13 遗留，`feature/l10n-p5-2-scheduler-grants`，独立 worktree `l10n-p5-2-grants`，基线 `feature/l10n-full-cps-parity@1da37bf`） | 零 schema migration，纯 `infra/postgres/grants.sql` 列级补授权。根因：L10N P5（`1da37bf`）让 `scheduler/index.ts` 的 `main()` 每 tick 调 `queryActiveLocales(prisma)`（`src/lib/locale/active-locales.ts`，`prisma.article.groupBy`，`where` 复用 `src/lib/seo/sitemap.ts` 的 `activePublicArticleWhere`，经 `src/server/publication/visibility.ts` 的 `PUBLIC_ARTICLE_RECORD` 关系过滤到 `novel`/`promo_link`），但 `grants.sql` 从未给 `scheduler_app` 授予这三张表的任何权限——X8 uat 真机复现 `scheduler` 容器崩溃重启（`permission denied for table article`，42501），`up` 的健康门禁卡死、`release-identity` 从未 commit（详见本轮验收进度记录，已 `spawn_task` 登记后由本轮修复并撤销该 chip）。修复前用 `DEBUG=prisma:query`（`scheduler_app` 角色本身，对着真实 X8 postgres 抓包，而非只读 Prisma schema 猜测）捕获这次调用的真实 SQL，逐列核对：`article` 需 `locale, deleted_at, status, novel_id, promo_link_id`；`novel`（别名 j0）需 `id, deleted_at, status`；`promo_link` 被 LEFT JOIN 两次（`PUBLIC_ARTICLE_RECORD` 的 `promoLink:{is:{status:"fetched"}}` 与 `activePublicArticleWhere` 自身追加的 `promoLink:{is:{deletedAt:null,OR:[...]}}}`，Prisma 无法合并成一次 JOIN，但两次都指向同一张物理表同一组列）需 `id, novel_id, status, deleted_at, web_url, app_url`；`seo_visibility` 全程不出现——核对 `docker-compose.yml` 的 `scheduler` 服务块，其 `environment` 从未声明 `FEATURE_ARTICLE_SEO_VISIBILITY`，`isArticleSeoVisibilityEnabled` 读到 unset 按 `false` 处理，故该列不进 `WHERE`，按最小授权纪律未多授。三条列级 `GRANT SELECT (...) ON <table> TO scheduler_app;` 加在既有 `schedule_run/cron_run/generic_task/generic_task_item` 那条授权之后，仿 PR6 lane E 给 `site_setting` 开的同款窄口子先例（同一文件、同一"scheduler 不豁免最小权限"纪律）。字典对齐：`database-schema-dictionary.jsonl` 的**表级** `read_roles`（`db:public:article`/`novel`/`promo_link` 三条 `record_kind:"table"` 记录）本就列了 `scheduler_app`（验收代理据此称"字典已声明"），但核对同一文件 `channel_account_credential` 等多处表级记录后确认：表级 `read_roles` 在本字典里一贯是"谁曾以任何形式触达过这张表"的粗粒度快照，并非"被授予了整张表 SELECT"的精确声明（`channel_account_credential` 表级同样列着 `scheduler_app`，但 `grants.sql` 从未给它任何权限）——真正与 `site_setting.id`/`carousel_config_json`（PR6 lane E 先例）对齐的精度层是**字段级**记录，而这三张表在字段级此前完全没有任何一条列出 `scheduler_app`，即验收代理"字典已声明该权限"的说法只在粗粒度表级成立、字段级实际是空的。按 PR6 lane E 的字段级先例补齐：`db:public:article:{locale,deleted_at,status,novel_id,promo_link_id}`、`db:public:novel:{id,deleted_at,status}`、`db:public:promo_link:{id,novel_id,status,deleted_at,web_url,app_url}` 共 14 条字段记录的 `read_roles` 追加 `scheduler_app`（插在 `worker_app` 之后，同既有排序惯例）；未改动三张表的表级记录（本就存在、本行不新增歧义）；未处理本轮顺带发现的 `promo_link.web_url`/`app_url` 两条字段记录 `read_roles` 缺 `web_app`（`grants.sql` 140 行明明已授权）——与本轮无关的既有字典漂移，如实记录不顺手修。 | Claude（Sonnet 5，L10N P5.2 施工） | `npm run typecheck`（0 error）；`npm run lint`（0 error，4 条既有 warning 数量不变）；`npm run test:ui`（141 files / 2377 passed）；`npm run test:backend`（220 files，219 passed / 1 failed——唯一失败 `tests/backend/publish-gate/no-bypass.test.ts`，同签名 `scripts/s1-exact-target-structural-smoke.ts` raw SQL 探测，历轮多次登记的既有基线失败，与本轮无关）；`node scripts/check-database-dictionary-drift.mjs --static`（`{"status":"ok","models":52,"recordCount":1208,"activeCount":1138}`，字段级编辑不改变行数/模型数）。新增 `tests/backend/database/active-locales-grants.test.ts`（7 例：三张表列集合静态断言 + 禁止整表/写权限负向断言 + 一条"扫描 `scheduler/index.ts` 真实（非仅静态可达）Prisma 调用面 ⊆ `grants.sql` 已授权表"的结构性回归守卫，后者用 `prisma/schema.prisma` 的 `@@map` 自解析 model→table，避免硬编码漂移）。两条变异均已在真实文件上做过、贴红、用 `Edit` 精确回滚（**不是** `git checkout --`——本轮踩过一次坑：第一次删 `article` 那条 GRANT 后用 `git checkout -- infra/postgres/grants.sql` 回滚，误将文件整体打回未加本轮授权的 HEAD 版本，随即发现并用 `Edit` 补回三条 GRANT，`git diff` 核对逐字复原）：①删 `article` 那条 `GRANT SELECT (...) ON article TO scheduler_app;` → 列集合断言与结构性守卫两处均转红；②在 `scheduler/index.ts` 的 `main()` 里加一行 `await prisma.channelAccount.findMany({ take: 1 })` → 结构性守卫检出 `channel_account` 不在 `grants.sql` 授权集合内转红，`Edit` 精确删回该行后 `git diff scheduler/index.ts` 为空。真机 X8 uat 实证（runtime dir `p2-06-main-merge/.tmp/x8-production-like`，与验收代理共享，`down`（未 `--purge`，卷保留）后从本 worktree `X8_LEVEL=uat DOCKER_BUILDKIT=0 COMPOSE_DOCKER_CLI_BUILD=0 up`，镜像 `cps-novel:0.1.0-1da37bf` 已存在未重建）：六服务（postgres/web/worker/scheduler/nginx/backup-timer）全部 `Up ... (healthy)`、`RestartCount=0`；`release-identity.json` 由脚本自身 commit（非本轮手工代跑，`gitCommit` 与 `imageRef` 均对得上）；两域 `/api/health` 均 200 `ok:true` `commit:1da37bf...`；`docker logs` 自重启后再无 `permission denied`/`42501`。用 `enqueueHomeCarouselCron`（该函数自身文档注明的"ops 强制入队"用途，非绕过）以 `scheduler_app` 角色真实凭证跑通全链路：`getHomeCarouselConfig`→`queryActiveLocales` 返回 `["en"]`（X8 当前仅 en 有已发布+已就绪推广链接的内容，符合前序 UAT 轮记录的真实数据状态）→`enqueueHomeCarouselCron` 返回 `status:"enqueued"`；SQL 复核 `generic_task_item.target_id="2026-09-11:en"`（`${businessDate}:${locale}` 格式，每个 active locale 恰一条）。角色读写实测（`scheduler_app` 自身口令，从 secrets 文件读、未进 argv/日志）：`SELECT` 已授权列成功（三表各 3 行）；`SELECT *`、`article.body`、`novel.title`、`promo_link.upstream_code` 均 `permission denied for table <t>`；`UPDATE`/`INSERT` 三表全部 `permission denied for table <t>`。**顺带发现、未处理、已 `spawn_task`**：同一次真实 `enqueueHomeCarouselCron` 触发后，Worker 侧 `computeHomeCarouselInTx` 的 finalize 步骤对 `home_carousel_serving.createMany()` 报 `ConnectorError`（`errorKind:"finalize_failed"`，`sqlState`/`prismaCode`/`constraint` 均为 `null`，不是权限错误），`generic_task_item.status` 落 `failed`——核对该 runtime 此前 09-05/09-06/09-07/09-09 四条历史 `home_carousel.compute.v1` 记录同一失败签名，确认是本轮之前就存在、与 `scheduler_app` 授权无关的 Worker 侧既有缺陷，未在本行范围内修复。 |
| 2026-09-11 | `home_carousel_serving.source` schema-contract-drift 修复（`fix/carousel-serving-source-check`，独立 worktree `carousel-source-fix`，基线 `feature/l10n-full-cps-parity@c37451d`） | 一条迁移 `20260912100000_carousel_serving_source_check_fix`：`DROP CONSTRAINT`+`ADD CONSTRAINT` 同名替换 `home_carousel_serving_source_check`，允许值集从 `('manual','automatic')` 改为 `('manual','new_novel','recency')`。根因（上一行 2026-09-11 L10N P5.2 changelog 末段已顺带发现并 `spawn_task` 登记、未处理）：`20260803090000_p1_initial_schema:1247` 装的 CHECK 只放行 `manual`/`automatic` 两桶，但 `src/server/home-carousel/service.ts`（`computeHomeCarouselInTx`，`154-157` 行区间）从落地起就只写细分值 `"manual" | "new_novel" | "recency"` 进 `home_carousel_serving.source`——`automatic` 这个值从未被任何代码路径产出过，导致只要一次 compute 的合并结果里包含哪怕一条自动候选行（`new_novel`/`recency`），整条 `homeCarouselServing.createMany()`（单条多行 `INSERT`，一行违反即整条语句失败，连同同一调用里的 manual 行一起报废）就以 PostgreSQL 23514 报错，Prisma 未把它映射成已知 `P*` 码，落到 Worker 侧就是 `sqlState`/`prismaCode`/`constraint` 全 `null` 的裸 `ConnectorError`。修复前核对 CPS 基线澄清"该按哪条改"：`3a76877:src/lib/home-carousel-merge.ts:134,154`（`mergeCarouselServingInTx` 把 `source:"manual"`/`source:candidate.source`——即 `new_drama|recency|revenue`——直接写进 serving）+ `3a76877:prisma/schema.prisma:808-826`／`3a76877:prisma/migrations/20260705090000_v770_home_carousel_pr1b/migration.sql`（CPS 该列 `"source" TEXT NOT NULL`，SQLite 无 CHECK 约束）：CPS 自己的 serving 列本就承载细分值、且没有任何限制其取值的约束，不存在"CPS 也是两桶"的先例。据此选择**对齐 CPS**（放宽 CHECK 匹配代码，而非收窄代码匹配 CHECK）：`src/server/home-carousel/service.ts` 与后台 `carousel-manager.tsx`（`SOURCE_LABEL = {manual:"人工位", new_novel:"新书位", recency:"最近更新"}`——从落地起就没有过 `automatic` 标签）零改动，**只改数据库契约层**。同步：`src/domain/database-statuses.ts` 的 `CAROUSEL_SOURCES` 从 `["manual","automatic"]` 改 `["manual","new_novel","recency"]`（补文档注释）；`tests/backend/home-carousel/support.ts` 的 `FakeHomeCarouselDb.homeCarouselServing.createMany` 补三条约束模拟（source∉CAROUSEL_SOURCES／`position<=0`／`(locale,position)` 与既有行或同批次内重复，均对齐迁移 SQL 里 `home_carousel_serving` 的完整约束清单，全批次 all-or-nothing——不满足任一条时不污染 `this.serving`，镜像真实单条多行 `INSERT` 语义）；新增 `tests/backend/database/carousel-serving-source-check-static.test.ts`（`CAROUSEL_SOURCES`↔修复迁移 CHECK 子句一致性守卫 + `service.ts` 里实际写入 `merged` 数组的 source 字面量⊆`CAROUSEL_SOURCES` 静态守卫，两条守卫互不依赖，任一边单独漂移都会各自转红）与 `tests/backend/home-carousel/serving-source-check.test.ts`（直接对 fake 的六个正/负例，含"批内一行违反连累整批"）；`tests/integration/tasks/p1-13-postgres-acceptance.test.ts` 的 `frozenChecks.home_carousel_serving_source_check` 字典项同步更新（该文件 `P1_13_DATABASE_TEST` 门控，本轮未跑）；`database-schema-dictionary.jsonl` 的 `db:public:home_carousel_serving:source`（字段）与 `db:public:home_carousel_serving:home_carousel_serving_source_check`（约束）两条记录原地更新 `enum_or_check`/`business_meaning`，`introduced_in_migration` 保持原值不变、`evidence` 追加本迁移文件引用（同 P2-02B/C-27/L10N-P3 的"CHECK 原地演进不 supersede"先例）；`port-registry.md` 新增一条登记本次 CPS 基线核对结论与偏离说明。**只读复现**（X8 uat `cps-novel-x8-local-postgres-1`，`worker_app` 角色真实口令+密码不回显、经 docker 网络 `cps_novel_x8_runtime` TCP+scram 连接，单个 `BEGIN;...;ROLLBACK;` 事务，修复前）：`home_carousel_serving` 当时 0 行（与 Worker 侧持续 finalize 失败一致）；`INSERT ... source='automatic'` 成功（`INSERT 0 1`）；同事务内 `INSERT ... source='recency'` 报 `ERROR: new row for relation "home_carousel_serving" violates check constraint "home_carousel_serving_source_check"`（23514）；`ROLLBACK` 后行数复核仍为 0，零持久化写入。 | Claude（Sonnet 5，施工） | `npm run typecheck`（0 error）；`npm run lint`（0 error，与改动前基线警告数量相同）；`npm run test:ui`；`npm run test:backend`（唯一允许失败 `tests/backend/publish-gate/no-bypass.test.ts`，同签名 `scripts/s1-exact-target-structural-smoke.ts`，历轮多次登记的既有基线失败，与本轮无关）。三条变异逐条做过、贴红、`git checkout -- <文件>` 回滚、核对 `git status`：①`service.ts` 把 `manual` 分支写死改成字面量 `"automatic"` → `carousel-serving-source-check-static.test.ts` 的写入点静态守卫红 + `serving-source-check.test.ts`/`compute.test.ts`/`merge.test.ts` 里任何触发该分支的用例经 fake CHECK 模拟红（双红）；②`CAROUSEL_SOURCES` 追加一个迁移 CHECK 里没有的值 → 迁移一致性守卫红；③在①的基础上同时把 fake 的 CHECK 模拟去掉（`createMany` 改回不做任何校验直接 push）→ 静态守卫依旧红（证明检测不是单靠 fake 的运行期模拟，而是独立的源码文本比对）。**一次性 PostgreSQL 16 容器 `migrate deploy` 幂等重放 + 双向 `migrate diff --exit-code` 零差异 + `check-database-dictionary-drift.mjs`（非 `--static`）均待后续执行方补跑**，本行状态到那之前不得视为"已验证"，与 C-24/C-27/C-30A/L10N-P3 四行同一措辞、同一约束。 |
| 2026-09-11 | `home_carousel_auto_batch.status` schema-contract-drift 修复（X8 轮 2a 实证，`fix/carousel-batch-status-check`，独立 worktree `carousel-batch-status-fix`，基线 `feature/l10n-full-cps-parity@11c60ff`） | **无 schema migration——代码对齐既有 CHECK**（与上一行 `home_carousel_serving.source` 修复不同：那次是 CHECK 本身发明错了两桶、放宽 CHECK 去对齐一直正确的代码；这次是 CHECK 本身自 P1 建库起就对——`home_carousel_auto_batch_status_check` 装的 `pending`/`processing`/`completed`/`failed` 四态是 P1 数据字典冻结口径，`database-schema-dictionary.jsonl` 的 `db:public:home_carousel_auto_batch:status` 记录本就准确描述这四态的业务含义，未改也不需要改——是代码写错了终态字面量，不是 CHECK 需要追认代码）。根因：`src/server/home-carousel/service.ts` 的 `computeHomeCarouselInTx`（原 158 行，紧跟 `home_carousel_serving.createMany` 之后）把批次终态写成字面量 `status: "success"`——`success` 从未是 `home_carousel_auto_batch_status_check` 的成员，本函数是单事务多语句写入（先 `homeCarouselServing.createMany`/`homeCarouselAutoCandidate.createMany`，最后才 `homeCarouselAutoBatch.update` 终态），这条终态 UPDATE 自身的 23514 把同一事务里刚写的 serving/candidate 行、连同批次行自己，一并 `ROLLBACK` 报废——与上一行同一根因类别（写入点字面量漂移出冻结 CHECK 的允许集）在同一张表族的第二例，本轮据此把 CHECK 模拟与静态守卫从"仅 `home_carousel_serving`"泛化到四表全覆盖。核对 CPS 基线（`3a76877:src/lib/home-carousel-compute.ts:228,272,284`）：CPS 同一列同样 SQLite `TEXT NOT NULL DEFAULT 'pending'`、**没有任何 CHECK 约束**，终态直接写 `status:"success"`/`status:"failed"`——若照搬上一行"CPS 无 CHECK 就放宽本仓 CHECK"的处理方式，这里该放行 `success`；但四态是数据字典冻结口径，**本轮不放宽 CHECK**，选四态里语义最贴近 CPS `"success"` 的 `"completed"`（终态、成功，与既有 `carousel_batch_status_created_idx` 索引设计的状态机形状一致），核对结论与决策差异详见 `port-registry.md` N-11 行。修复：①`src/server/home-carousel/service.ts` 该行终态改 `status: "completed" satisfies CarouselBatchStatus`（`create` 时既有的 `status: "pending"` 同步补 `satisfies CarouselBatchStatus`；候选/合并数组的 `source` 字面量同步补 `satisfies CarouselSource`、`merged` 数组类型标注从 `source: string` 收紧为 `source: CarouselSource`——`satisfies` 编译期擦除、不改运行期字面量，字面量本身仍由静态守卫+假库两道运行期防线兜底，三层不互斥）；②`src/domain/database-statuses.ts` 补 `CarouselBatchStatus`/`CarouselSource` 两个 `ValueOf` 类型（此前四张表里唯独这两个值集常量有常量无类型，`CAROUSEL_BATCH_STATUSES`/`CAROUSEL_SOURCES` 值集本身未新增/未删减一个字面量）；③`tests/backend/home-carousel/support.ts` 的 `FakeHomeCarouselDb` 四表全覆盖 CHECK/唯一键模拟（此前只有 `home_carousel_serving` 一张表有模拟）：`homeCarouselAutoBatch.create`/`.update` 补 `home_carousel_auto_batch_status_check`；`homeCarouselAutoCandidate.createMany` 补 `carousel_candidate_rank_check`（`rank>0`）+ 两条唯一键（`batch_id,locale,rank`／`batch_id,novel_id`），**不**模拟 `source` CHECK——两份迁移文件核对确认 `home_carousel_auto_candidate.source` 本就没有 CHECK 约束，模拟一个不存在的约束会是假阳性；`homeCarouselManualSlot.create`/`.update` 补 `carousel_manual_position_check`（`position>0`）+`carousel_manual_window_check`（`starts_at<ends_at`）+ 两条局部唯一索引（`(locale,position)`/`(locale,novel_id)`，均只在 `enabled AND deleted_at IS NULL` 时生效，镜像 PostgreSQL 局部索引语义）；`prismaCheckViolationError` 从硬编码 `home_carousel_serving`/`createMany` 泛化为按 `relation`/`method` 参数化，四表复用同一构造函数。④`tests/backend/database/carousel-serving-source-check-static.test.ts` 重命名为 `tests/backend/database/carousel-check-static.test.ts`（`git mv`）并从"仅 `home_carousel_serving.source` 一列"扩为四表全 CHECK 静态守卫：新增按**约束名**（而非列名）解析 CHECK 子句的 `parseCheckValuesByConstraint`（发现并绕开一个陷阱——`20260803090000_p1_initial_schema` 里 `CHECK ("status" IN (...))` 这个列名在十几张表上重复出现，原有按列名取"第一个匹配"的 `parseCheckValues` 若直接扩到 `status` 列会悄悄取错表，已改按约束名精确匹配规避）；新增 `home_carousel_auto_batch.status` 迁移一致性守卫 + 写入点字面量静态守卫（`"success"` 回归钉死断言）；新增 `home_carousel_auto_candidate.source` 写入点字面量守卫（无 DB CHECK backing，纯字典/应用层约定，文档化"为何这里没有 CHECK 却仍要守"）；新增四表 CHECK 完整清单盘点测试（约束名集合逐表精确比对，任何一张表未来新增/删除一条 CHECK 而没人同步这份清单即转红）；新增一条断言 `worker/handlers/home-carousel.ts` 不直接写四张表任何一张（确认该 handler 只通过 `computeHomeCarouselInTx` 的 `protectedWrite` 闭包写入，写入点静态守卫因此只需扫 `service.ts` 一个文件即可穷尽）。⑤新增 `tests/backend/home-carousel/carousel-check-simulation.test.ts`（16 例，直接对假库 `homeCarouselAutoBatch`/`homeCarouselAutoCandidate`/`homeCarouselManualSlot` 三表的 CHECK/唯一键模拟做正负例，同 `serving-source-check.test.ts` 对 `home_carousel_serving` 的既有覆盖风格）；`compute.test.ts` 新增一条读回持久化批次行 `status` 字段实值的测试（此前全套用例只断言 `computeHomeCarouselInTx` 自身返回值的 `status:"success"` 字段——与本次修复的 DB 列完全是两回事的独立契约，此前从未真正验证过持久化的 `home_carousel_auto_batch.status` 落了什么值）。`database-schema-dictionary.jsonl`：**未改动**——CHECK 值集本身未变，`db:public:home_carousel_auto_batch:status` 记录本就准确，无漂移可同步。`port-registry.md` 新增 N-11 行登记本次 CPS 基线核对结论与"为何不照搬 N-10 的放宽 CHECK 处理方式"。**只读复现**（X8 uat `cps-novel-x8-local-postgres-1`，镜像 `cps-novel:0.1.0-11c60ff`，`worker_app` 角色真实口令+密码不回显、`docker exec -i` 经容器内 `psql` 连接，两个独立 `BEGIN;...;ROLLBACK;` 事务，修复前）：`home_carousel_auto_batch` 修复前基线 0 行（与 Worker 侧持续 finalize 失败一致，同上一行 `home_carousel_serving.source` 复现一致）；`INSERT ... status='success'` 报 `ERROR: new row for relation "home_carousel_auto_batch" violates check constraint "home_carousel_auto_batch_status_check"`（23514，`DETAIL` 原文列出完整候选行）；`ROLLBACK`；另一事务 `INSERT ... status='completed'` 成功（`INSERT 0 1`，同事务内 `SELECT` 可见该行），`ROLLBACK`，事务外复核仍 0 行——零持久化写入，两次读写均未 `COMMIT`、未重建/重启任何容器。 | Claude（Sonnet 5，施工） | `npm run typecheck`（0 error）；`npm run lint`（0 error，4 条既有 warning，与改动前基线数量逐字相同、文件路径也相同——均非本轮改动文件）；`npm run test:ui`（141 files / 2377 passed）；`npm run test:backend`（223 files，222 passed / 1 failed——唯一失败 `tests/backend/publish-gate/no-bypass.test.ts`，同签名 `scripts/s1-exact-target-structural-smoke.ts` raw SQL 探测，历轮多次登记的既有基线失败，与本轮无关）。三条变异逐条做过、贴红、精确 `Edit` 回滚（本轮改动文件只含预期修复+新增测试，无需 `git checkout --` 整体回滚）、核对 `git status` 回滚前后文件集合逐字一致：①`service.ts` 终态写死改回字面量 `"success"`（去掉 `satisfies CarouselBatchStatus`）→ 假库红（`compute.test.ts`/`merge.test.ts` 共 13 例经 `homeCarouselAutoBatch.update` 的 CHECK 模拟报错）+ 静态守卫红（`carousel-check-static.test.ts` 写入点字面量断言 `['pending','success']` 不等于期望的 `['pending','completed']`），双红；②`CAROUSEL_BATCH_STATUSES` 追加一个迁移 CHECK 里没有的值 `"cancelled"` → 迁移一致性守卫红（2 例：常量本身的四态断言 + 常量↔CHECK 子句一致性断言）；③`service.ts` 给 `homeCarouselAutoCandidate.createMany` 的 `source` 字面量改成 CHECK 外的值 `"trending"`（该列本无 DB CHECK，故假库对四表的 CHECK 模拟保持全绿——唯一意外掉红的是一条不相关的既有业务断言 `expect(...).toBe("new_novel")`，因为它恰好断言了这个具体值，非 CHECK 机制捕获）→ 静态守卫单独转红（`carousel_auto_candidate.source` 写入点字面量断言），证明四表泛化确实覆盖到了"无 DB CHECK 但有应用层字典约定"的这张表，而不是只对有真实 CHECK 的三张表生效。**一次性 PostgreSQL 16 容器 `migrate deploy` 幂等重放 + 双向 `migrate diff --exit-code` 零差异 + `check-database-dictionary-drift.mjs`（非 `--static`，本轮无字典改动理论上应为 no-op，仍需实跑确认）均待后续执行方补跑**，本行状态到那之前不得视为"已验证"，与 C-24/C-27/C-30A/L10N-P3/上一行 `home_carousel_serving.source` 五行同一措辞、同一约束。 |
| 2026-09-11 | `worker_app` RETURNING 权限缺口全仓审计与修复（X8 轮 2c 实证，`fix/grants-returning-select`，独立 worktree `grants-returning-fix`，基线 `feature/l10n-full-cps-parity@75dbdee`） | 零 schema migration，纯 `infra/postgres/grants.sql` 表级补授权。与上两行（X8 轮 2a/2b）同一诊断脉络的第三个、也是最后一个根因：`computeHomeCarouselInTx` 的 CHECK 漂移两处都修完后，`worker/handlers/home-carousel.ts` 触发的真实事务仍在 `home_carousel_change_log` 的 `homeCarouselChangeLog.create()` 处以 `permission denied for table home_carousel_change_log` 整体回滚——`grants.sql` 对 `worker_app` 该表只给了 `GRANT INSERT`（`PR6 fix lane E` 有意为之，见 `2026-09-05` 那行"`home_carousel_change_log` 保持 INSERT-only 不变"，该决定本身就是本次的根因：作者当时判断 INSERT-only 已够用，未意识到 Prisma 的 RETURNING 语义）。机制：Prisma 的 `.create()`/`.update()`/`.upsert()`/`.delete()` 在没有显式 `select` 时一律编译成携带 `RETURNING <模型全部标量列>` 的 SQL，PostgreSQL 对 RETURNING 的每一列都按 SELECT 权限校验、与该语句实际写了哪些列无关——只有 INSERT/UPDATE/DELETE 而无 SELECT 的角色因此连一条最简单的 `.create()` 都执行不了，首行即失败并回滚整个事务；`createMany`/`updateMany`/`deleteMany` 不受影响（driver 的 `rowCount` 直接给出 `{count}`，Prisma 不为这三者附加 RETURNING）。全仓审计方法：解析 `grants.sql` 得到逐 (role, table) 权限集合，与 `prisma/schema.prisma` 的 `@@map` 做 model↔table 映射，再静态扫描 `worker/**`、`scheduler/**`、`src/server/**`、`src/lib/**` 的 `.create(`/`.update(`/`.upsert(`/`.delete(`/`.createManyAndReturn(` 调用并逐个回溯调用者（web route/action vs `worker/handlers/*` vs `scheduler/index.ts` 的真实 import 关系，而非猜测），按运行进程归属角色判定风险。结论：`worker_app` 有 INSERT/UPDATE 但零 SELECT 的表共七张——`home_carousel_change_log`（确认根因）、`indexnow_outbox_attempt`/`indexnow_outbox`（`worker/handlers/indexnow-delivery.ts` 的 `indexNowOutboxAttempt.create()`/`indexNowOutbox.update()` 四处，同样是每次投递都会踩的现役缺口，非本轮才发现——只是从未被真机流量以外的方式暴露过）、`tracking_event`/`schedule_run`/`cron_run`/`article_template`（`worker_app` 早有 INSERT/UPDATE 授权但代码库里目前没有任何 worker 侧调用点——`tracking_event`/`article_template` 现由 `web_app` 写且该角色已有完整 SELECT，`schedule_run`/`cron_run` 现由 `scheduler_app` 自授权 `SELECT,INSERT,UPDATE` 覆盖，均安全——判定为"授权早已存在、代码尚未使用"的潜伏地雷，与已确认现役的三张表同一模式、按同一纪律一并补齐，不留到下一次真机流量撞上才修）。审计过程中同时核对了 `web_app`/`analyst_ro` 侧全部列级缩窄授权（`channel_account_credential`/`channel_credential_active_fingerprint`/`credential_change_log`/`novel_source_item`/`side_effect_intent` 五处 SELECT 列表不含全部模型列），确认对应写入点（`src/server/credentials/service.ts` 三条 `$executeRaw` 原生 SQL 无 RETURNING、`credentialChangeLog.create` 显式 `select:{id:true}`、`content-creation/service.ts`/`task-admin/service.ts` 两处均用 `updateMany`）全部已用原生 SQL 或显式 `select`/`updateMany` 规避了 RETURNING，零同类风险，不需要改动。修复：`GRANT SELECT ON TABLE home_carousel_change_log, indexnow_outbox_attempt, tracking_event, indexnow_outbox, schedule_run, cron_run, article_template TO worker_app;` 一条表级授权（延续 `worker_app` 在本文件里"零列级 SELECT、只用表级"的既有风格，七张表都不含对 `worker_app`隐藏的敏感列）。字典核对：`database-schema-dictionary.jsonl` 对这七张表**全部字段级** `read_roles` 记录本就已经列了 `worker_app`（逐字段脚本核验，零遗漏）——`grants.sql` 落后于字典记录的既定意图，而不是反过来；因此**本行未改动字典**，`grants.sql` 的补授权是把实际权限拉齐到字典早已声明、但从未真正 GRANT 过的状态。**只读复现**（X8 uat `cps-novel-x8-local-postgres-1`，镜像 `cps-novel:0.1.0-75dbdee`，`worker_app` 角色真实口令+密码不回显、`docker exec -i` 经容器内 `psql` 连接，修复前）：`INSERT INTO home_carousel_change_log (locale, action, actor_type) VALUES ('en','grants_returning_probe','system') RETURNING id;` 报 `ERROR:  permission denied for table home_carousel_change_log`；另起一个独立事务，同一条 INSERT 去掉 `RETURNING id` 后 `INSERT 0 1` 成功（证明失败专属于 RETURNING，不是 INSERT 本身被拒绝）；两个事务均 `ROLLBACK`，`postgres` 超级用户事务外复核 `action='grants_returning_probe'` 行数为 0，零持久化写入，全程未 `COMMIT`、未重建/重启任何容器。**追加发现、未在本行修复、已 `spawn_task` 登记**：(a) `src/app/(admin)/home-carousel/_actions.ts` 的 `saveManualCarouselSlotAction`/`deleteManualCarouselSlotAction`（经 `upsertHomeCarouselManualSlot`/`deleteHomeCarouselManualSlot`，`src/server/home-carousel/service.ts:210-236`）以 `web_app` 身份直接调用 `deps.db.$transaction`，同样对 `home_carousel_change_log` 执行 `.create()`——但 `web_app` 对该表从未被授予任何 INSERT，是比本行 RETURNING 缺口更基础的"整条 INSERT 都会 permission denied"，与本轮审计范围（"已有写权限、缺 SELECT"）性质不同、发现时点也晚于本任务的既定范围（X8 轮 2c 只界定到 `worker_app`），未随手并入本次 grants.sql 改动；(b) `prisma/schema.prisma` 的 `article_novel_rebind_preview`/`_batch`/`_batch_item` 三张 C-30A 表在 `grants.sql` 里没有任何角色的任何授权，但 `src/server/article-rebind/{preview,batch}.ts` 已有 `articleNovelRebindPreview.create()`/`articleNovelRebindBatch.create()` 等实现并已接线到 `src/app/(admin)/articles/_actions.ts`（`FEATURE_ARTICLE_NOVEL_REBIND` fail-closed 双闸挡着，flag 关闭时不可达）——一旦该 flag 开启，这条链路会在最基础的 INSERT 权限上失败，而不是本行修的 RETURNING 层。两项均判定为超出本任务字面范围（本任务界定为"补齐已确认的 RETURNING 缺口"，不是"重新设计整条新特性的授权模型"），未经额外评审不擅自扩大 grants.sql 改动面。 | Claude（Sonnet 5，施工） | `npm run typecheck`（0 error）；`npm run lint`（0 error，4 条既有 warning，路径与改动前基线逐字相同，均非本轮改动文件）；`npm run test:ui`（141 files / 2377 passed）；`npm run test:backend`（224 files，223 passed / 1 failed——唯一失败 `tests/backend/publish-gate/no-bypass.test.ts`，同签名 `scripts/s1-exact-target-structural-smoke.ts` raw SQL 探测，历轮多次登记的既有基线失败，与本轮无关）。新增 `tests/backend/database/grants-returning.test.ts`（5 例：解析 `grants.sql` 得到逐角色 `{select,insert,update,delete}`，再用真实 import 关系从 `worker/` 的每个文件出发做 BFS 可达性分析——非手工文件清单——扫描可达文件的 `.create(`/`.update(`/`.upsert(`/`.delete(`/`.createManyAndReturn(` 调用并按 `@@map` 映射回物理表，断言 `worker_app` 对每一张这样的表都有 SELECT；`scheduler_app` 侧因文件级 BFS 对共享文件`src/server/home-carousel/service.ts`会带出误报（`operation_audit`，经 `web_app`-only 的 `upsertHomeCarouselManualSlot` 混入同一文件），本文件按其自身注释说明的理由未对 `scheduler_app` 做同款 BFS，改为对其已知的四张自授权表做窄口精确断言，避免与 `active-locales-grants.test.ts` 已有的声明级精细扫描重复）；同步修订 `tests/backend/database/carousel-grants.test.ts` 原先断言 `home_carousel_change_log` 对 `worker_app` "INSERT-only 且禁止 SELECT" 的用例——该断言本身就是本次要修的 bug 在测试里的镜像，反转为断言 SELECT 存在。两条变异均在真实文件上做过、贴红、精确复原（用 `cp` 备份/还原而非 `git checkout --`，因改动集混有已计划内的 grants.sql 编辑，整体 checkout 会连带丢弃）：①从新增的表级 SELECT 里删掉 `home_carousel_change_log` → `carousel-grants.test.ts`/`grants-returning.test.ts` 各一处断言转红（3 例失败）；②在 `worker/handlers/home-carousel.ts` 的 `createHomeCarouselHandler` 里插入一行对 `admin_login_attempt`（`worker_app` 完全零权限的表）的 `db.adminLoginAttempt.create(...)`，同时临时加一条 `GRANT INSERT ON TABLE admin_login_attempt TO worker_app;` 复现"新表拿到 INSERT 但没跟着给 SELECT"的场景——`grants-returning.test.ts` 的代码扫描断言（而非仅按表名硬编码的静态断言）单独转红，证明扫描机制本身、不只是对已知七表的名单、能抓到未来任何新增的同类调用点；`cp` 还原两个文件后 `diff` 确认逐字复原、`git status --short` 只剩本行预期改动。 |

| 2026-09-11 | `web_app` 缺失授权收尾 + RETURNING 守卫扩至 `web_app`（X8 轮 2c 收尾，`fix/grants-returning-select`，独立 worktree `grants-returning-fix`，基线 `fix/grants-returning-select@0072833`，即上一行"追加发现、未在本行修复"登记的两项 (a)(b) 的实际修复） | 零 schema migration，纯 `infra/postgres/grants.sql` 表级补授权 + 测试新增，与上一行同一性质。上一行 X8 轮 2c 的全仓审计判定范围"只界定到 `worker_app`"，把两个发现顺手记录但未修：(a) `src/app/(admin)/home-carousel/_actions.ts` 的 `saveManualCarouselSlotAction`/`deleteManualCarouselSlotAction`（经 `upsertHomeCarouselManualSlot`/`deleteHomeCarouselManualSlot`，`src/server/home-carousel/service.ts:212,235`）以 `web_app` 身份对 `home_carousel_change_log` 执行 `.create()`，但 `web_app` 对该表从未有任何 INSERT——比 RETURNING 缺口更基础的"整条 INSERT 都 permission denied"；(b) `article_novel_rebind_preview`/`_batch`/`_batch_item` 三张 C-30A 表在 `grants.sql` 里没有任何角色的任何授权，而 `src/server/article-rebind/{preview,batch}.ts` 已实现并已接线到 `src/app/(admin)/articles/_actions.ts`（`FEATURE_ARTICLE_NOVEL_REBIND` fail-closed 双闸挡着，flag 关闭时不可达，但 grants 本身必须先到位）。本行修复两项：①`GRANT INSERT ON TABLE home_carousel_change_log TO web_app;`（该表对 `web_app` 是 INSERT-only，同 `operation_audit` 一样没有 UPDATE 调用点；`web_app` 已有该表 SELECT，故只需补 INSERT）。②按 `src/server/article-rebind/{preview,batch}.ts` 真实调用图逐表核对角色与动词后补齐：三张表的读写全部经 `src/app/api/admin/_lib/deps.ts` 导出的共享 `web_app` PrismaClient 执行（`_actions.ts` 把 `prisma` 传入每一个 rebind 函数），仓内 `worker/`、`scheduler/` 均无任何 rebind 相关引用（`grep -rln "article-rebind\|articleNovelRebind"` 验证），判定为纯 `web_app` 特性；`GRANT SELECT ON TABLE article_novel_rebind_preview, article_novel_rebind_batch, article_novel_rebind_batch_item TO web_app, analyst_ro;`（`analyst_ro` 配对沿用本文件对其余 `web_app` 自有业务表的既有惯例）+ `GRANT INSERT ON TABLE` 三表 `TO web_app;` + `GRANT UPDATE ON TABLE article_novel_rebind_batch, article_novel_rebind_batch_item TO web_app;`（`preview` 无任何 `.update()`/`.upsert()` 调用点，故不给 UPDATE）+ `GRANT DELETE ON TABLE article_novel_rebind_preview TO web_app;`（仅 `preview` 有 `cleanupExpiredRebindPreviews` 的 `.deleteMany()`，`batch`/`batch_item` 无任何删除调用点，故不给 DELETE）。逐表调用点：`article_novel_rebind_preview` = `buildRebindBatchPreview` 的 `.create()`（`preview.ts:749`）+ `cleanupExpiredRebindPreviews` 的 `.deleteMany()`（`preview.ts:889`，经 `submitRebindBatch` 调用，`batch.ts:672`）；`article_novel_rebind_batch` = `submitRebindBatch` 的 `.create()`（`batch.ts:218`）+ 七处 `.updateMany()` 租约/状态迁移调用点（`batch.ts:275,284,289,302,534,538,547`）；`article_novel_rebind_batch_item` = `submitRebindBatch` 的 `.createMany()`（`batch.ts:237`）+ 四处 `.updateMany()` claim/fence/terminal 调用点（`batch.ts:319,351,374,391`）——均无任何裸 `.delete()`。字典核对：`database-schema-dictionary.jsonl` 对 `home_carousel_change_log`（14 条记录，表级+全部字段级）与三张 `article_novel_rebind_*` 表（76 条记录，表级+全部字段级）本就已经把 `web_app` 列在 `write_roles`（`home_carousel_change_log` 额外含 `worker_app`；三张 rebind 表的 `read_roles`/`write_roles` 逐字为 `["web_app","analyst_ro"]`/`["migration_owner","web_app"]`）——与上一行同一结论："`grants.sql` 落后于字典早已声明的意图，而不是反过来"，**本行未改动字典**。守卫扩展：`tests/backend/database/grants-returning.test.ts` 新增第二个 `describe` 块，BFS 根目录从 `worker/` 平移到 `src/app/(admin)/` + `src/app/api/admin/`（`web_app` 的完整请求入口集），断言与 `worker_app` 一致但更严格一档——不仅 `hasSelect`，还断言 `hasAnyWrite`（`web_app` 此前的缺口恰是"有 SELECT 无 INSERT"和"全无授权"两种形状，单查 SELECT 会在这两处都假阳性通过）。BFS 产生一个已知假阳性并显式登记（`WEB_APP_WRITE_EXEMPTIONS` 常量,连同存活性自检——断言该表确实仍被 BFS 标记、且 `web_app` 确实仍无写权限，避免登记过期或掩盖真实缺口）：`home_carousel_auto_batch`——`src/server/home-carousel/service.ts` 被 `web_app` 侧 `_actions.ts` 引入，但该表的 `.create()`/`.update()` 只出现在 `computeHomeCarouselInTx` 内部，该函数只被 `worker/handlers/home-carousel.ts` 调用（`grep -rl computeHomeCarouselInTx src/ worker/ scheduler/` 验证零 web_app 可达路径），与上一行 `scheduler_app`/`operation_audit` 假阳性同一根因（共享文件的文件级 BFS 无法区分函数归属）。`article_novel_rebind_batch_item` 全部写入都是 `createMany`/`updateMany`（本文件 BFS 方法集按既有约定不含 `Many` 变体），落在 BFS 扫描范围外，故追加窄口手工核验断言（同 `scheduler_app` 已知写面的既有处理方式）。**只读复现**（X8 uat `cps-novel-x8-local-postgres-1`，镜像 `cps-novel:0.1.0-75dbdee`——本行不改运行时代码，沿用上一行镜像，`web_app` 角色真实口令+密码不回显、`docker exec -i` 经容器内 `psql` 连接，四个独立 `BEGIN;...;ROLLBACK;` 事务，修复前）：`INSERT INTO home_carousel_change_log (locale, action, actor_type) VALUES (...)` 报 `ERROR:  permission denied for table home_carousel_change_log`；`INSERT INTO article_novel_rebind_preview (id, created_by) VALUES (...)` 报 `ERROR:  permission denied for table article_novel_rebind_preview`；`INSERT INTO article_novel_rebind_batch (id, created_by) VALUES (...)` 报 `ERROR:  permission denied for table article_novel_rebind_batch`；`SELECT count(*) FROM article_novel_rebind_batch_item` 报 `ERROR:  permission denied for table article_novel_rebind_batch_item`（`batch_item` 此前连 SELECT 都没有,不止是本行修的 INSERT/UPDATE 缺口）；四事务均 `ROLLBACK`，`migration_owner` 事务外复核三张探测过的表 `grants_followup_probe` 标记行数均为 0，零持久化写入，全程未 `COMMIT`、未重建/重启任何容器。 | Claude（Sonnet 5，施工） | `npm run typecheck`（0 error）；`npm run lint`（0 error，4 条既有 warning，路径与改动前基线逐字相同，均非本轮改动文件）；`npm run test:ui`（141 files / 2377 passed）；`npm run test:backend`（224 files，223 passed / 1 failed——唯一失败 `tests/backend/publish-gate/no-bypass.test.ts`，同签名 `scripts/s1-exact-target-structural-smoke.ts` raw SQL 探测，历轮多次登记的既有基线失败，与本轮无关）。`grants-returning.test.ts` 由 5 例扩至 12 例（新增 7 例 web_app 覆盖：BFS 非空 canary、主断言循环、豁免存活性自检、两项修复锁定、现役调用点 canary、`batch_item` 窄口 pin、`analyst_ro` 配对断言），单独跑 12/12 通过，`carousel-grants.test.ts` 未受影响（6/6）。两条变异逐条做过、贴红、`cp` 备份/`md5`/`diff` 精确复原（同上一行手法，因改动集混有本行计划内的 `grants.sql` 编辑，`git checkout --` 会连带丢弃）：①从 `web_app` 的 INSERT 列表中删除 `home_carousel_change_log` 这一行整条注释掉 → 新 `describe` 块 2 例转红（主断言循环 + 两项修复锁定,均报 `home_carousel_change_log` 无 INSERT/UPDATE/DELETE）；②把 `article_novel_rebind_preview` 从三表 INSERT 列表中摘除、同时注释掉其专属 DELETE 授权（只动 `preview` 一张表，`batch`/`batch_item` 授权不变，验证"任一表"颗粒度）→ 同 2 例转红（`article_novel_rebind_preview` 报无 INSERT/UPDATE/DELETE）；两次均 `md5`/`diff` 确认 `grants.sql` 逐字节复原到变异前，`git status --short` 复原后只剩本行预期改动的两个文件（`infra/postgres/grants.sql`、`tests/backend/database/grants-returning.test.ts`）。 |
| 2026-09-11 | withdraw UI 永久「处理中」兜底 + novel takedown grants 缺口（Owner 批准窄范围修复 lane，`fix/withdraw-ui-guard-and-takedown-grants`，独立 worktree `withdraw-ui-takedown-grants`，基线 `feature/l10n-full-cps-parity@6fd7bf9`） | 零 schema migration，`infra/postgres/grants.sql` 表级/列级补授权 + 两处 UI try/catch/finally 兜底 + 新增 `*Many` 统计级权限守卫。**并发note**：开工前 `git worktree list`/`git branch --list` 核对发现两条相关但独立的在建分支——`fix/grants-returning-select`（812c0da）其 HEAD 已是本 worktree 基线的祖先（`git merge-base --is-ancestor 812c0da HEAD` 为真，非并发，只是历史）；`fix/grants-takedown-chapter-writes`（`f88c18d`+复核 `81380ce`，独立 worktree `grants-takedown-chapter-fix`，同基线 `6fd7bf9`）**是真正的并发分支**，`git show --stat` 只读核对其内容与本行的 grants/守卫部分高度重合（同一根因、同一两条 GRANT、同一 22→24 条 `*Many` 守卫用例思路），按施工纪律未合并、未依赖，独立复现独立验证，留 Owner/Codex 裁决取舍其一。**UI 部分**：`src/app/(admin)/articles/_components/article-list.tsx` 的 `confirmWithdraw()` 与 `src/app/(admin)/novels/_components/publish-lifecycle-panel.tsx` 的 `runRightsTransition()` 此前都在 `await <ServerAction>(...)` 外层没有 try/catch，且各自被 `void confirmWithdraw()`/`void runRightsTransition(pending)` 触发——一次 reject（Owner 撤回 `de` 文章时实际观测到的场景，同会话曾八次 "Failed to find Server Action" 的 stale bundle）会跳过 `await` 之后包括 `setWithdrawBusy(false)`/`setBusy(false)` 在内的每一行，`ConfirmDialog` 的 `pending` 永久 `true`，按钮永久显示"处理中…"且禁用，无提示、无法重试。补 try/catch/finally：`finally` 恒重置 busy；`catch` 按错误文本区分展示两条已写好的中文提示之一（stale bundle 专属措辞 vs 通用网络/版本措辞），对话框刻意不在 catch 里关闭（保留已填理由，刷新后可直接重按确认按钮重试）。判断错误文本时用 `String(error)` 而非 `error.message`——`tests/ui/admin-secret-boundary.test.tsx` 既有的 "never reads a server-authored message off an error" 守卫按源码文本禁止在 `src/app/(admin)`/`src/features/admin-ui`/`src/components/ui` 任何文件里出现字面 `error.message`/`envelope.message`（首次实现时踩中，`npm run test:ui` 报红后改用 `String(error)`）；`String(error)` 拿到同样的判别信号（`Error.prototype.toString()` 含 message），且只用于在两条已经写好的中文文案间二选一，从不回显原始异常文本，与该守卫"错误文案由前端自己拥有、不透传服务端/运行时原文"的立意不冲突。**Grants 部分**：`applyNovelRightsTransition` 的 takedown 分支（`src/server/publish-gate/service.ts:763-786`）在同一 `$transaction` 内按 chunk 先 `tx.novelChapterContent.deleteMany({where:{novelChapterId:{in:idChunk}}})` 再 `tx.novelChapter.updateMany({where:{id:{in:idChunk}},data:{status:"withdrawn"}}）`——唯一调用路径是 `_actions.ts` 的 `takedownNovelAction → takedownNovel → applyNovelRightsTransition`，全程走共享 `web_app` PrismaClient（`grep -rn "takedownNovel\|applyNovelRightsTransition" src worker scheduler` 零 worker/scheduler 命中）。`web_app` 此前对两表只有 SELECT，novel_chapter 无 UPDATE、novel_chapter_content 无 DELETE（该表 DELETE 之前是 worker_app-only）——takedown 一本有章节的已发布 Novel 会在第一个 chunk 上 `42501 permission denied`。修复：`GRANT UPDATE (status, updated_at) ON novel_chapter TO web_app;`（`novel_chapter.updatedAt` 带 Prisma `@updatedAt`，`updateMany` 隐式也写这一列，故列集是两列而非一列）+ `GRANT DELETE ON TABLE novel_chapter_content TO web_app;`（`deleteMany` 的 `WHERE` 只需已有的 SELECT，不需要额外列级 INSERT/UPDATE——`grep -rn "novelChapterContent\.\|novelChapter\.\(create\|update\|upsert\|delete\)" src/app/(admin) src/app/api/admin src/server` 核对这是 web_app 对两表仅有的写入，无 createMany）。字典核对：`database-schema-dictionary.jsonl` 两表相关字段的 `write_roles` 本就已列 `web_app`（`novel_chapter.status`/`updated_at` 两条字段记录 + `novel_chapter_content` 表级记录与除 `body` 外全部字段记录），`grants.sql` 落后于字典早已声明的意图——本行**未改动字典**，与 2026-09-11 前两行同一套结论。**`*Many` 守卫**：`tests/backend/database/grants-returning.test.ts` 新增第五个 `describe` 块，`.createMany(`/`.updateMany(`/`.deleteMany(` 三个方法（既有 RETURNING 守卫的 `WRITE_METHODS`/`detectWriteMethodsByTable` 两套正则均结构性排除这三者，因为它们不带隐式 RETURNING）分别要求 INSERT/UPDATE/DELETE，复用既有 `workerFiles`/`webAppFiles` BFS 结果，对 worker_app 与 web_app 两进程都判定（既有 RETURNING 守卫只对 worker_app 做过这层扫描，`web_app` 段是上两行才补的；本行是`*Many`维度首次覆盖两进程）。扫描出的假阳性（`home_carousel_serving::createMany/deleteMany`、`home_carousel_auto_batch::createMany`、`home_carousel_auto_candidate::createMany` 均在 worker-only 的 `computeHomeCarouselInTx` 内，独立重新验证 `grep -rl computeHomeCarouselInTx src/ worker/ scheduler/` 三命中同既有结论；`site_setting::updateMany` 对 worker_app 是共享文件假阳性，`worker/handlers/indexnow-delivery.ts` 只导入该文件的 `getIndexNowDeliveryConfig`/`isIndexNowConfigured`，从未导入真正调用 `updateMany` 的 `updateAdminSiteSetting`）逐条登记进 `WEB_APP_BULK_METHOD_EXEMPTIONS`/`WORKER_APP_BULK_METHOD_EXEMPTIONS`，各配存活性自检。**新发现两个真实、超出本行范围的 worker_app 权限缺口**（`novel_canonical_tag::deleteMany`，经 `worker/handlers/novel-tag-backfill.ts → replaceAutoTagSnapshotInTransaction`，`service.ts:425`；`indexnow_outbox_attempt::updateMany`，`worker/handlers/indexnow-delivery.ts:210` 直接调用）——与本行 novel takedown 无关的既有缺口，未顺手修复，登记进 `WORKER_APP_BULK_METHOD_KNOWN_GAPS`（区别于假阳性登记，附存活性自检防止静默过期或被误用来掩盖新缺口）并各自 `spawn_task` 一条独立任务登记（`task_0740dcbb`/`task_44b43776`）。**只读复现**（X8 uat `cps-novel-x8-local-postgres-1`，本行未改动运行时镜像，`web_app` 角色真实口令+密码不回显、`docker exec -i` 经容器内 `psql` socket trust 连接，两个独立 `BEGIN;...;ROLLBACK;` 事务，`WHERE false` 使权限在计划期即被拒绝、与实际行数无关，修复前）：`UPDATE novel_chapter SET status = status WHERE false;` 报 `ERROR:  permission denied for table novel_chapter`；另一事务 `DELETE FROM novel_chapter_content WHERE false;` 报 `ERROR:  permission denied for table novel_chapter_content`；`has_table_privilege` 矩阵确认修复前 `web_app` 对 `novel_chapter` UPDATE=false、对 `novel_chapter_content` DELETE=false（SELECT 均已为 true）。两事务均 `ROLLBACK`，零持久化写入，全程未 `COMMIT`、未重建/重启任何容器；"修复后"状态未对共享 X8 实例做真实重放（`grants.sql` 需 `migration_owner` 单事务重放整份文件，超出本行"只读 BEGIN…ROLLBACK"授权范围），以 `grants-returning.test.ts` 对更新后 `grants.sql` 的解析断言（见下方门禁）代为验证。 | Claude（Sonnet 5，施工） | `npm run typecheck`（0 error）；`npm run lint`（0 error，4 条既有 warning，路径与数量同改动前基线逐字相同）；`npm run test:ui`（143 files / 2404 passed，含新增 4 例 stale-bundle/网络错误 UI 用例）；`npm run test:backend`（224 files，223 passed / 1 failed——唯一失败 `tests/backend/publish-gate/no-bypass.test.ts`，同签名 `scripts/s1-exact-target-structural-smoke.ts` raw SQL 探测，历轮多次登记的既有基线失败，与本轮无关）；`grants-returning.test.ts` 单独 24/24（原 24 个用例即为新增后的总数，含本行新增的 8 个 `*Many` 守卫用例）。四条变异逐条在真实文件上做过、贴红、用 `cp` 备份/`diff` 逐字节复原（非 `git checkout --`，改动集混有本行计划内编辑）：①`confirmWithdraw` 删 `finally` → 新增的 stale-bundle UI 用例转红（按钮 `disabled` 停在 `true`，`waitFor` 超时）；②`confirmWithdraw` 的 `catch` 清空为静默吞错 → 同一用例转红（`getByRole("status")` 找不到元素）；③`grants.sql` 注释掉 `novel_chapter` 的 UPDATE 授权 → `grants-returning.test.ts` 两处断言转红（主循环 + 本行"锁定修复"pin）；④在 web 可达文件（`src/app/api/admin/_lib/deps.ts`）以注释形式加入一行对无 DELETE 授权表 `operation_audit` 的 `.deleteMany(` → 同一 `describe` 块的主循环断言单独转红，证明守卫按扫描结果通用判定而非仅认已登记的两张表。四次均 `diff`/`md5` 确认逐字节复原，复原后 `git status --short` 只剩本行预期改动的六个文件（`infra/postgres/grants.sql`、两个组件、`grants-returning.test.ts`、两个 UI 测试文件）。`docs/governance/port-registry.md` 同步新增一节登记两项均为"不适用（非搬运）"——UI 兜底是内部 bugfix，grants 是字典对齐，均不构成从 CPS 搬运符号。 |
| 2026-09-11 | PR6 M5 遗留缺陷修复：幂等路径污染事务 + 配置保存权限（X8 轮 2d 实证，`fix/carousel-idempotency-and-config-update`，独立 worktree `carousel-idem-config-fix`，基线 `feature/l10n-full-cps-parity@812c0da`） | 零 schema migration，纯 `src/server/home-carousel/service.ts` 代码修复 + 假库/静态守卫/测试新增。⑥ `computeHomeCarouselInTx` 原用 `homeCarouselAutoBatch.create` + `catch(P2002 → skipped_duplicate)` 处理 `unique_key` 幂等冲突——CPS 同款写法在 SQLite 上安全，但该函数恒运行在 worker 共享的 `protectedWrite` 事务内，PostgreSQL 上一条语句抛错即把整个事务标记 `25P02 aborted`，其后 `finalizeTaskItem`/`guardedFinalize` 的 `tx.$executeRaw` 终态写入必然失败，同日二次触发即 `finalize_failed`。改用 `createMany({ data: [batchRow], skipDuplicates: true })`（编译为 `INSERT ... ON CONFLICT (unique_key) DO NOTHING`），Postgres 在 SQL 层面无异常地消解冲突，`count === 0` 判定 `skipped_duplicate`；不再有 `P2002` 字面量 catch。批次 id 改为调用前 `randomUUID()` 现铸，`count === 1` 成功路径无需回读。写法参照仓内先例 `worker/handlers/promo-link-claim.ts` 的 `ensurePromoLinkRow`（同一"开放事务内避免异常"理由，`upsert`/`ON CONFLICT DO UPDATE`）。⑦ `updateHomeCarouselConfig` 原用 `tx.siteSetting.upsert`——`grants.sql` 只给 `web_app` 对 `site_setting` 列级 UPDATE（含 `carousel_config_json`），INSERT/DELETE 是 `migration_owner`-only；PostgreSQL 的 `INSERT ... ON CONFLICT` 无论冲突分支是否真正落到 UPDATE，执行该语句本身都要求 INSERT 权限，`web_app` 身份下必然 `permission denied for table site_setting`。改用 `tx.siteSetting.update`，先 `findUnique` 守卫读：行缺失时抛 `SiteSettingNotSeededError`（复用 `src/server/site-settings/service.ts` 既有类，不新造并行错误码，且该类已接入 `respond.ts` 的后台错误信封）而非静默 upsert 出一行——`site_setting` 是 bootstrap 播种的单例行（`id` 固定 1，`site_setting_singleton_check` CHECK 禁止任何其它行），本函数没有需要真正"创建"该行的合法路径。两处均在 `docs/governance/port-registry.md` 登记为 N-12/N-13（PG/SQLite 语义差异或本仓自有选择，均标注非可直接搬运 CPS 字面实现）。假库同步：`tests/backend/home-carousel/support.ts` 的 `homeCarouselAutoBatch` 新增 `createMany`（保留 `create` 供 `carousel-check-simulation.test.ts` 的 CHECK 保真测试直接使用），`skipDuplicates:true` 时对 `unique_key` 冲突跳过而非抛错、仍对 `status` CHECK 逐行校验；`siteSetting` 的 `upsert` 换成 `update`（新增 `siteSettingSeeded` 开关模拟行缺失，`findUnique` 返回 `null`、`update` 抛 `P2025`,镜像真实 Prisma/Postgres 行为）。静态守卫：`tests/backend/database/carousel-check-static.test.ts` 新增 `describe` 块——剥离 `//`/`/* */` 注释后断言 `service.ts` 不含可执行的 `"P2002"` 字面量（历史注释里的 `P2002` 提及不受影响，守卫自检"字符串确实还在文件里、只是在注释里"防止形同虚设）；既有的 `autoBatchStatusLiteralsWrittenByService` 标记词由 `homeCarouselAutoBatch.create(` 改 `homeCarouselAutoBatch.createMany(`。`tests/backend/database/grants-returning.test.ts` 新增 `web_app` 段的按方法精确守卫（`create`/`upsert` 须有 INSERT、`update` 须有 UPDATE、`delete` 须有 DELETE，而非既有 `hasAnyWrite` 的"任一写权限即可"粗粒度——`site_setting` 的 `upsert` 缺陷正是被 `hasAnyWrite`（因既有列级 UPDATE 为真）掩盖过的形状），随附一枚方法级豁免（`side_effect_intent`/`create`，经 `@/lib/tasks` 桶文件 `export *` 带入的假阳性，`web_app` 侧确实无 INSERT、创建只在 `worker/handlers/promo-link-claim.ts` 内，与本任务 ⑥/⑦ 无关，登记原因见该文件内联注释）。**只读复现**（X8 uat `cps-novel-x8-local-postgres-1`，镜像 `cps-novel:0.1.0-812c0da`——本行不改运行时镜像，`web_app`/`worker_app` 角色真实口令+密码不回显、`docker exec -i` 经容器内 `psql` 连接，`BEGIN;...;ROLLBACK;` 事务）：⑦ 以 `web_app` 身份 `UPDATE site_setting SET carousel_config_json = carousel_config_json WHERE id = 1;` 成功（`UPDATE 1`）；同一事务内 `INSERT INTO site_setting (id) VALUES (1) ON CONFLICT (id) DO UPDATE SET carousel_config_json = site_setting.carousel_config_json;` 报 `ERROR:  permission denied for table site_setting`。⑥ 以 `worker_app` 身份在一个事务里对同一 `unique_key` 连续 `INSERT`（无 `ON CONFLICT`）：第二条报 `duplicate key value violates unique constraint "home_carousel_auto_batch_unique_key_key"`，再下一条语句即报 `current transaction is aborted, commands ignored until end of transaction block`；另起一个事务改用 `INSERT ... ON CONFLICT (unique_key) DO NOTHING`：第二条返回 `INSERT 0 0`（无异常），再下一条语句正常执行。全部事务均 `ROLLBACK`，零持久化写入，全程未 `COMMIT`、未重建/重启任何容器。 | Claude（Sonnet 5，施工） | `npm run typecheck`（0 error）；`npm run lint`（0 error，4 条既有 warning，与改动前基线逐字相同，均非本轮改动文件）；`npm run test:ui`（141 files / 2377 passed）；`npm run test:backend`（224 files，223 passed / 1 failed——唯一失败 `tests/backend/publish-gate/no-bypass.test.ts`，同签名 `scripts/s1-exact-target-structural-smoke.ts` raw SQL 探测，历轮多次登记的既有基线失败，与本轮无关）。三条变异逐条做过、贴红、精确回滚（改动集混有本行计划内的 `service.ts`/`support.ts`/测试编辑，`git checkout --` 会连带丢弃，故用 Edit 精确回退到变异前文本，非 `git checkout`）：①`computeHomeCarouselInTx` 改回 `create` + `catch(P2002)` → `compute.test.ts`/`cron.test.ts` 幂等相关用例转红（`createMany` call-trace 断言 + `carousel-check-static.test.ts` 的 `createMany(` 标记词定位失败 + P2002 静态守卫转红）；②`updateHomeCarouselConfig` 改回 `siteSetting.upsert` → `config.test.ts` 新增用例转红（`siteSetting.upsert` call-trace 出现）+ `grants-returning.test.ts` 的 `site_setting` 按方法守卫转红（`upsert` 需 INSERT，`web_app` 无）；③假库 `homeCarouselAutoBatch.createMany` 的 `skipDuplicates` 分支改为恒插入（忽略 `args.skipDuplicates`）→ `compute.test.ts` 的幂等测试转红（`db.batches.size` 变 2、`second.status` 变 `"success"` 而非 `"skipped_duplicate"`）。三次均回退后 `git status --short` 复原为改动前预期集合。 |
