...
| `FEATURE_NOVEL_TAG_AUTO` | `false` | resolver、tagging task factory/worker | 允许读取 auto layer 与创建 auto task；不等于授权生产写入。2026-09-26 ADR 已批准同一开关同时控制公开投影是否纳入 qualified auto，与后台生效标签共用；工单 7 已实现 dark 接线与公开投影，当前关闭，关闭时公开结果须与修订前逐字节一致。见 [Owner amendment](../adr/ADR-P2-06-5-TAGGING-V3.md#2026-09-26-owner-amendment--public-tag-projection-includes-qualified-auto-cps-parity)；批量首次定类仅对本批新建 ID 按 5,000 分段建任务；此登记不表示已上线或 G7 已授权。 |
