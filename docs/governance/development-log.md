# 开发日志（LEGACY · 已于 2026-09-07 冻结）

> **本文件已冻结，不要再追加。** 最后一条是 2026-09-07。
>
> 冻结原因：手工流水账在本项目的节奏下撑不住。2026-09-07 到 2026-09-18 之间
> 有 10 次以上实质改动（WAL 保留 Gate、catalog finalize recovery、发布与 Preview 解耦、
> 渠道账号级故障刹车、批量发布审计编号冲突修复、文章列表跨页全选）——一条都没进来。
> 一份没人维护却被人信任的日志，比没有日志更危险。
>
> **替代方案**（见 [`AI_WORKFLOW.md`](AI_WORKFLOW.md)）：
> - "改了什么" → `CHANGELOG.md`，由 `node scripts/generate-changelog.mjs --write`
>   从 git log + tag + trailer 生成，不可能与代码漂移；
> - "为什么这么定" → `docs/adr/`；
> - "谁用什么模型做的" → commit 的 `Agent:` / `Model:` trailer。
>
> 2026-09-07 之前的内容原样保留作历史参考，**刻意不做考古式补写**：
> 事后追记的细节没有当时的证据支撑，只会制造另一种看起来可信的失真。
