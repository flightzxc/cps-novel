# 开发日志（自 v0.3.0 起：只记发版级条目）

> **规则（2026-09-23，Owner 裁决）**：2026-09-07 的冻结原因是手工逐改动流水账
> 跟不上节奏——冻结后到 2026-09-18 之间仍有 10 次以上实质改动
> （WAL 保留 Gate、catalog finalize recovery、发布与 Preview 解耦、渠道账号级
> 故障刹车、批量发布审计编号冲突修复、文章列表跨页全选）一条都没记进来，
> 一份没人维护却被人信任的日志比没有日志更危险。这个诊断没有变；变的是
> 记录粒度——本文件自 **v0.3.0** 起解冻，但**只记发版级条目**，不恢复逐改动手记：
>
> - 一次正式发版（预生产或生产）记一条，新条目在最上面。模板沿用 CPS 短剧格式：
>
>   ```
>   ### YYYY-MM-DD HH:mm - 作者/Agent
>
>   **变更类型**：
>   **背景**：
>   **变更内容**：
...
>   ```
>
> - "改了什么"（逐 commit）仍看 `CHANGELOG.md`——由
>   `node scripts/generate-changelog.mjs --write` 从 git log + tag + trailer 生成，
>   **禁止手改**；"为什么这么定"仍看 `docs/adr/`。这两条口径不变，详见
>   [`AI_WORKFLOW.md`](AI_WORKFLOW.md)"发版治理"一节。
> - **不做考古式补写**：2026-09-07 冻结至本次发版（v0.3.0）之间的历史空档
>   不追记——事后补的细节没有当时的证据支撑，只会制造另一种看起来可信的失真。
>   本轮 v0.3.0 的发版级条目，在正式发布（预生产或生产）完成后由发版执行者
>   按实际结果写入。
> - Notion 同步：有 Notion 连接时，先写本文件 → 再同步 Notion 台账 → 再读回核对；
>   没有连接时，由发版执行者按发版治理规程生成交接提示词，交 Owner 转 ChatGPT
>   写入 Notion 手账。
>
> 2026-09-07 之前的逐改动流水账内容原样保留作历史参考；2026-09-19 那一条是
> 冻结期内的例外记录，同样原样保留，不追溯删除、不重新归类。
