...
- 开发线 `integration/v0.4.1-2026-09-24`（基于 `v0.4.0` 收官提交 `c4bdcbb`），合入
  `feat/catalog-sync-promo-link-status-filter` @ `42114d9`（待办 B-4，经 Opus 复核）+ 版本身份升到 0.4.1。
- 范围：**随部署生效**——目录同步页新增"推广链接状态"筛选（未领取 / 已领取 / 人工核对中，与书目状态、语种
  取交集）；领取资格列把有码书显示为"已有推广码"、人工核对中的书显示为"人工核对中"；全选提交后枚举与界面
  筛选共用同一判定。无数据库迁移、无 grants 变更。
- 目的：Owner 要求在正式领取剩余约 7.6 万本之前完成；第二级 UAT 用"已建立书目 + 俄语 + 未领取 + 全选"一并验证。
...
| --- | ---: | --- | --- | --- | --- |
| `v0.4.1` | 发布时填写 | PATCH | 目录同步页推广链接状态筛选（未领取 / 已领取 / 人工核对中）与"已有推广码"显示（待办 B-4）。详见"当前快照" | 发布时填写 | 🟡 准备中（未发布） |
